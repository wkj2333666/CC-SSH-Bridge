    ...
    pub fn discover() -> BridgeResult<Self> {
        ...
        if bin_directory.file_name() != Some(OsStr::new("bin")) {
            return Err(BridgeError::invalid_config(
                "install must run from the packaged bin/cc-ssh-bridge binary",
            ));
        }
...
fn install_lock_path(resolved: &ResolvedInstall) -> BridgeResult<PathBuf> {
    ...
    Ok(home.join(".cc-ssh-bridge.install.lock"))
}
...
pub async fn install_user(layout: InstallLayout, apply: bool) -> BridgeResult<InstallReport> {
    ...
    let applied = async {
        if mcp == Presence::Absent {
            ...
            if let Err(failure) = cc_add_checked(&resolved).await {
                ...
                if failure.outcome == MutationOutcome::Unknown {
                    return Err(BridgeError::io(
                        "Claude Code MCP add outcome is unknown; rollback incomplete",
                    ));
                }
...
pub async fn uninstall_user(layout: InstallLayout, apply: bool) -> BridgeResult<InstallReport> {
    ...
    let removed = async {
        if mcp == Presence::Matching {
            if cc_get(&resolved).await? != Presence::Matching {
                return Err(BridgeError::invalid_config(
                    "MCP entry changed after uninstall preflight",
                ));
            }
            match cc_remove_checked(&resolved).await {
                Ok(()) => mcp_removed = true,
                ...
                Err(_) => {
                    return Err(BridgeError::io(
                        "Claude Code MCP removal outcome is unknown; rollback incomplete",
                    ));
                }
            }
        }
...
async fn rollback_uninstall(
    ...
    if mcp_removed {
        match cc_add_checked(resolved).await {
            Ok(()) => {}
...
async fn rollback_install(
    ...
    if journal.mcp_added {
        match cc_get(resolved).await {
            Ok(Presence::Matching) => match cc_remove_checked(resolved).await {
                Ok(()) => {}
                Err(error) if error.outcome == MutationOutcome::Applied => {}
                Err(_) => failed = true,
            },
...
fn resolve_layout(layout: InstallLayout) -> BridgeResult<ResolvedInstall> {
    ...
    let cc_executable = canonical_secure_cc_executable(&layout.cc_executable)?;
    let skill_source = canonical_secure_directory(&layout.skill_source)?;
    for required in ["SKILL.md", "references/operations.md"] {
...
fn validate_package_layout(
    ...
    if plugin_directory.file_name() != Some(OsStr::new(".claude-plugin")) {
        return Err(BridgeError::invalid_config(
            "plugin manifest is not under .claude-plugin",
        ));
    }
    let bundle = plugin_directory
        .parent()
        .ok_or_else(|| BridgeError::invalid_config("plugin manifest has no bundle root"))?;
    if plugin_manifest != bundle.join(".claude-plugin/plugin.json")
        || binary != bundle.join("bin/cc-ssh-bridge")
        || mcp_manifest != bundle.join(".mcp.json")
        || skill_source != bundle.join("skills/remote-ssh-ops")
    {
        return Err(BridgeError::invalid_config(
    ...
    if plugin.get("name").and_then(serde_json::Value::as_str) != Some("cc-ssh-bridge") {
        return Err(BridgeError::invalid_config(
            "plugin manifest does not describe cc-ssh-bridge",
        ));
    }
    ...
    if server.get("command").and_then(serde_json::Value::as_str) != Some("./bin/cc-ssh-bridge")
...
async fn cc_get(resolved: &ResolvedInstall) -> BridgeResult<Presence> {
    let output = run_cc(resolved, ["mcp", "get", MCP_NAME]).await?;
    if output.status == 1 && cc_missing(&output.stderr) {
...
fn mcp_matches(stdout: &[u8], binary: &Path) -> BridgeResult<bool> {
...
async fn cc_add_checked(resolved: &ResolvedInstall) -> Result<(), MutationError> {
    ...
    let failure = match attempted {
        Ok(output) if output.status == 0 => {
            BridgeError::io("`claude mcp add` reported success without a matching final state")
        }
        Ok(_) => BridgeError::io("`claude mcp add` failed"),
        Err(error) => error,
    };
    let outcome = match cc_get(resolved).await {
...
async fn cc_remove_checked(resolved: &ResolvedInstall) -> Result<(), MutationError> {
    let attempted = run_cc(resolved, ["mcp", "remove", "-s", "user", MCP_NAME]).await;
    let command_succeeded = matches!(&attempted, Ok(output) if output.status == 0);
    let failure = match attempted {
        Ok(output) if output.status == 0 => {
            BridgeError::io("`claude mcp remove` reported success without an absent final state")
        }
        Ok(_) => BridgeError::io("`claude mcp remove` failed"),
        Err(error) => error,
    };
    let outcome = match cc_get(resolved).await {
...
async fn run_cc_os(
    ...
    run_local_command(LocalCommandSpec {
        executable: resolved.layout.cc_executable.clone(),
        arguments,
...
fn canonical_secure_cc_executable(path: &Path) -> BridgeResult<PathBuf> {
    validate_trusted_ancestors(path)?;
    ...
    if entry_metadata.file_type().is_symlink() {
        ...
        if !trusted_source_owner(entry_metadata.uid(), current_uid, root_uid) {
            return Err(BridgeError::invalid_config(
                "Claude Code CLI symlink must be owned by root or the current user",
            ));
        }
    } else {
...
fn hash_secure_skill_tree(root: &Path) -> BridgeResult<String> {
    ...
    hasher.update(b"cc-ssh-bridge-skill-tree-v1\0");

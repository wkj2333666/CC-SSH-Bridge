...
cc_snapshot_parent_stat_follow() {
    stat -L --printf='%f:%u:%a:%s:%d:%i:%h\n' -- "$1" 2>/dev/null
}
cc_snapshot_decimal_valid() {
...
cc_snapshot_stat_parse() {
    ...
    cc_snapshot_decimal_valid "$2" || return 1
    [ "${#3}" -le 4 ] || return 1
    case "$3" in ''|*[!0-7]*) return 1 ;; esac
    cc_snapshot_decimal_valid "$4" || return 1
    cc_snapshot_decimal_valid "$5" || return 1
    cc_snapshot_decimal_valid "$6" || return 1
    cc_snapshot_decimal_valid "$7" || return 1
    CC_STAT_TYPE=$1
    CC_STAT_UID=$2
    CC_STAT_MODE=$3
    CC_STAT_SIZE=$4
...
cc_snapshot_stat_valid() {
    cc_stat_line=$(cc_snapshot_stat "$1") || return 9
    cc_snapshot_stat_parse "$cc_stat_line"
}
cc_snapshot_parent_stat_follow_valid() {
    cc_stat_line=$(cc_snapshot_parent_stat_follow "$1") || return 9
    cc_snapshot_stat_parse "$cc_stat_line"
}
cc_snapshot_read() {
...
cc_snapshot_hash() (
    ...
    for cc_hash_line in $cc_hash_capture; do
        case "$cc_hash_line" in
            CC_DD_STATUS=*)
                [ "$cc_hash_dd_seen" -eq 0 ] || { cc_hash_valid=0; break; }
                cc_hash_dd_seen=1
                cc_hash_dd=${cc_hash_line#CC_DD_STATUS=}
                ;;
            CC_SHA_STATUS=*)
                [ "$cc_hash_sha_seen" -eq 0 ] || { cc_hash_valid=0; break; }
                cc_hash_sha_seen=1
                cc_hash_sha=${cc_hash_line#CC_SHA_STATUS=}
                ;;
            *'  -')
                [ "$cc_hash_digest_seen" -eq 0 ] || { cc_hash_valid=0; break; }
                cc_hash_digest_seen=1
                cc_hash_digest=${cc_hash_line%  -}
                ;;
            *) cc_hash_valid=0; break ;;
        esac
    done
...
cc_patch_snapshot_sentinel() (
    umask 077
    cc_sentinel_dir=$(mktemp -d "${TMPDIR:-/tmp}/cc-sentinel-patch-snapshot.XXXXXX" 2>/dev/null) || exit 9
    cleanup_cc_sentinel() {
    ...
    cc_snapshot_parent_stat_follow_valid "$cc_sentinel_parent" || exit $?
    cc_sentinel_parent_identity=$CC_STAT_DEVICE:$CC_STAT_INODE
    cc_snapshot_parent_stat_follow_valid "$cc_sentinel_parent_link" || exit $?
    [ "$CC_STAT_DEVICE:$CC_STAT_INODE" = "$cc_sentinel_parent_identity" ] || exit 1
    case "$CC_STAT_TYPE" in 4???) ;; *) exit 1 ;; esac
    cc_sentinel_file=$cc_sentinel_parent/file
    printf payload >"$cc_sentinel_file" || exit 9
    cc_snapshot_stat_valid "$cc_sentinel_file" || exit $?
    case "$CC_STAT_TYPE" in 8???) ;; *) exit 1 ;; esac
    [ "$CC_STAT_SIZE" = 7 ] || exit 1
    cc_sentinel_hash=$(cc_snapshot_hash "$cc_sentinel_file") || exit $?
    [ "$cc_sentinel_hash" = 239f59ed55e737c77147cf55ad0c1b030b6d7ee748a7426952f9b852d5a935e5 ] || exit 1
    cc_sentinel_content=$(cc_snapshot_read "$cc_sentinel_file") || exit 9
    [ "$cc_sentinel_content" = payload ] || exit 1
    cc_sentinel_link=$cc_sentinel_parent/link
    ln -s "$cc_sentinel_file" "$cc_sentinel_link" || exit 9
    cc_snapshot_stat_valid "$cc_sentinel_link" || exit $?
    case "$CC_STAT_TYPE" in a???) ;; *) exit 1 ;; esac
    if cc_snapshot_read "$cc_sentinel_link" >/dev/null 2>&1; then exit 1; fi
...
for cc_required_command in stat mktemp dd sha256sum ln rm mkdir cat; do
    command -v "$cc_required_command" >/dev/null 2>&1 || {
        printf 'CODE=CAPABILITY_MISMATCH\000CAPABILITY=safe_write\000' >&2
        exit 0
    }
done
cc_sentinel_status=0
cc_patch_snapshot_sentinel || cc_sentinel_status=$?
...
cc_snapshot_decimal_valid "$maximum_size" || exit 2
emit_one() {
...
cc_classify_unreachable_parent() {
    ...
    while :; do
        ...
        cc_snapshot_stat_valid "$cc_parent_candidate" || cc_parent_lstat_status=$?
        ...
        0)
            case "$CC_STAT_TYPE" in
                ...
                a???)
                    ...
                    cc_snapshot_parent_stat_follow_valid "$cc_parent_candidate" || cc_parent_follow_status=$?
...
cc_parent_line=$(cc_snapshot_parent_stat_follow "$parent") || {
    cc_classify_unreachable_parent
    exit 3
}
cc_snapshot_stat_parse "$cc_parent_line" || exit 3
...
target_size=$CC_STAT_SIZE
target_device=$CC_STAT_DEVICE
target_inode=$CC_STAT_INODE
target_mode=$CC_STAT_MODE
target_links=$CC_STAT_LINKS
target_mode_decimal=$((0$target_mode))
[ $((target_mode_decimal & 07000)) -eq 0 ] || emit_one WRITE_CONFLICT
cc_snapshot_decimal_le "$target_size" "$maximum_size" || emit_one REQUEST_TOO_LARGE

target_hash1_status=0
target_hash1=$(cc_snapshot_hash "$target") || target_hash1_status=$?
if [ "$target_hash1_status" -ne 0 ]; then
    if cc_snapshot_stat_valid "$target"; then
        case "$CC_STAT_TYPE" in 8???) ;; *) emit_one READ_CONFLICT ;; esac
        [ "$CC_STAT_SIZE:$CC_STAT_DEVICE:$CC_STAT_INODE:$CC_STAT_MODE:$CC_STAT_LINKS" = "$target_size:$target_device:$target_inode:$target_mode:$target_links" ] || emit_one READ_CONFLICT
...
cc_snapshot_read "$target" || target_read_status=$?
...
target_hash2=$(cc_snapshot_hash "$target") || emit_one READ_CONFLICT

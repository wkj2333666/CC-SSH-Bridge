...
cs() (
 ...
 b=$d/cc-sentinel-list-production;z=$d/z;ft=$b/ft;fr=$b/fr;lr=$b/lr;hr=$b/hr
 ...
 cmp -s "$d/e" "$o"||exit 12
 x=$(printf 'a\nb\000'|lx sh -c 'printf %s "$1"' cc-sentinel-list-xargs 2>/dev/null);s=$?
 printf x\000|lx sh -c 'exit 7' cc-sentinel-list-xargs >/dev/null 2>&1;q=$?
 [ "$s" -eq 0 ]&&[ "$q" -ne 0 ]&&[ "$x" = 'a
b' ]||exit 13
)
cs;s=$?
...
umask 077
t=$(mktemp -d /tmp/cc-ssh-list.XXXXXX)||exit 2
cleanup(){ rm -rf -- "$t"; }
...
[ "$#" -eq 0 ]||exit 65
printf %s "$n">"$c"
' cc-ssh-list "$f" "$M" <"$a" >"$b" 2>/dev/null
printf %s "$?">"$e"
) &
q=$!
exec 3<"$b"
...
cc_check_stat() (
    cc_stat_dir=$(mktemp -d /tmp/cc-sentinel-stat.XXXXXX 2>/dev/null) || exit 2
    cleanup_cc_stat() { rm -rf -- "$cc_stat_dir"; }
    trap cleanup_cc_stat EXIT HUP INT TERM
    cc_stat_file=$cc_stat_dir/file
    printf x >"$cc_stat_file" || exit 2
    chmod 640 "$cc_stat_file" || exit 2
    touch -d '@-1.123456789' -- "$cc_stat_file" || exit 2
    cc_stat_mode=$(stat --printf='%f' -- "$cc_stat_file" 2>/dev/null) || exit 1
    cc_stat_size=$(stat --printf='%s' -- "$cc_stat_file" 2>/dev/null) || exit 1
    cc_stat_seconds=$(stat --printf='%Y' -- "$cc_stat_file" 2>/dev/null) || exit 1
    cc_stat_human=$(stat --printf='%y' -- "$cc_stat_file" 2>/dev/null) || exit 1
    cc_stat_fraction=$(printf '%s' "$cc_stat_human" | cut -d. -f2 | cut -d' ' -f1)
    [ "$cc_stat_mode:$cc_stat_size:$cc_stat_seconds:$cc_stat_fraction" = \
      '81a0:1:-2:876543211' ]
)
cc_stat_status=0
cc_check_stat || cc_stat_status=$?
if [ "$cc_stat_status" -eq 1 ]; then
    printf 'CODE=CAPABILITY_MISMATCH\000CAPABILITY=stat_printf\000' >&2
    exit 0
fi
if [ "$cc_stat_status" -ne 0 ]; then exit 2; fi
cc_xargs_newline='line
name'
cc_xargs_out=$(printf 'line\nname\000' |
    xargs -0 -r sh -c 'printf %s "$1"' cc-sentinel-stat-xargs 2>/dev/null)
cc_xargs_ok=$?
printf 'x\000' |
    xargs -0 -r sh -c 'exit 7' cc-sentinel-stat-xargs >/dev/null 2>&1
cc_xargs_failure=$?
if [ "$cc_xargs_ok" -ne 0 ] || [ "$cc_xargs_failure" -eq 0 ] ||
   [ "$cc_xargs_out" != "$cc_xargs_newline" ]; then

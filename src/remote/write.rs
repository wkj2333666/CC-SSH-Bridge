...
cc_mutation_parent_stat_follow() {
    stat -L --printf='%f:%u:%a:%s:%d:%i:%h\n' -- "$1" 2>/dev/null
}

cc_mutation_mktemp() {
    mktemp --tmpdir="$1" .cc-ssh-bridge.XXXXXXXXXX
}

cc_mutation_stage() (
    on_cc_mutation_stage_signal() {
        trap - HUP INT TERM
        exit 125
    }
    trap on_cc_mutation_stage_signal HUP INT TERM
...
cc_mutation_decimal_valid() {
    case "$1" in ''|*[!0-9]*) return 1 ;; esac
    [ "${#1}" -le 20 ] || return 1
    [ "${#1}" -lt 20 ] && return 0
    cc_decimal_value=$1
    cc_decimal_limit=18446744073709551615
    while [ -n "$cc_decimal_value" ]; do
        cc_decimal_digit=${cc_decimal_value%"${cc_decimal_value#?}"}
        cc_decimal_limit_digit=${cc_decimal_limit%"${cc_decimal_limit#?}"}
        [ "$cc_decimal_digit" -lt "$cc_decimal_limit_digit" ] && return 0
        [ "$cc_decimal_digit" -gt "$cc_decimal_limit_digit" ] && return 1
        cc_decimal_value=${cc_decimal_value#?}
        cc_decimal_limit=${cc_decimal_limit#?}
    done
    return 0
}

cc_mutation_stat_parse() {
    cc_stat_line=$1
    case "$cc_stat_line" in ''|*[!0-9a-f:]*) return 1 ;; esac
    cc_stat_old_ifs=$IFS
    IFS=:
    set -- $cc_stat_line
    IFS=$cc_stat_old_ifs
    ...
    cc_mutation_decimal_valid "$2" || return 1
    [ "${#3}" -le 4 ] || return 1
    case "$3" in ''|*[!0-7]*) return 1 ;; esac
    cc_mutation_decimal_valid "$4" || return 1
    cc_mutation_decimal_valid "$5" || return 1
    cc_mutation_decimal_valid "$6" || return 1
    cc_mutation_decimal_valid "$7" || return 1
    CC_STAT_TYPE=$1
    CC_STAT_UID=$2
    CC_STAT_MODE=$3
    CC_STAT_SIZE=$4
...
cc_mutation_stat_valid() {
    cc_stat_line=$(cc_mutation_stat "$1") || return 9
    cc_mutation_stat_parse "$cc_stat_line"
}

cc_mutation_parent_stat_follow_valid() {
    cc_stat_line=$(cc_mutation_parent_stat_follow "$1") || return 9
    cc_mutation_stat_parse "$cc_stat_line"
}
...
cc_mutation_hash() (
        ...
        {
            {
                ...
                printf 'CC_DD_STATUS=%s\n' "$?" >&2
            } | sha256sum 2>/dev/null
            printf 'CC_SHA_STATUS=%s\n' "$?" >&2
        } 2>&1
    )
    cc_hash_dd=
    cc_hash_sha=
    ...
    cc_hash_dd_seen=0
    cc_hash_sha_seen=0
    cc_hash_digest_seen=0
    cc_hash_valid=1
    set -f
    IFS="$newline"
    for cc_hash_line in $cc_hash_capture; do
        case "$cc_hash_line" in
            CC_DD_STATUS=*)
                if [ "$cc_hash_dd_seen" -ne 0 ]; then
                    cc_hash_valid=0
                    break
                fi
                cc_hash_dd_seen=1
                cc_hash_dd=${cc_hash_line#CC_DD_STATUS=}
                ;;
            CC_SHA_STATUS=*)
                if [ "$cc_hash_sha_seen" -ne 0 ]; then
                    cc_hash_valid=0
                    break
                fi
                cc_hash_sha_seen=1
                cc_hash_sha=${cc_hash_line#CC_SHA_STATUS=}
                ;;
            *'  -')
                if [ "$cc_hash_digest_seen" -ne 0 ]; then
                    cc_hash_valid=0
                    break
                fi
                cc_hash_digest_seen=1
                cc_hash_digest=${cc_hash_line%  -}
                ;;
            *)
                cc_hash_valid=0
                break
    ...
    if [ "$cc_hash_dd_seen" -ne 1 ] || [ "$cc_hash_sha_seen" -ne 1 ]; then
        cc_hash_valid=0
    fi
    if [ "$cc_hash_valid" -ne 1 ]; then
        cc_hash_status=1
    elif [ "$cc_hash_dd" != 0 ] || [ "$cc_hash_sha" != 0 ]; then
        cc_hash_status=9
    elif [ "$cc_hash_digest_seen" -ne 1 ] || [ "${#cc_hash_digest}" -ne 64 ]; then
        cc_hash_status=1
    else
        case "$cc_hash_digest" in *[!0-9a-f]*) cc_hash_valid=0 ;; esac
        if [ "$cc_hash_valid" -eq 1 ]; then
            printf '%s\n' "$cc_hash_digest"
            cc_hash_status=0
        else
            cc_hash_status=1
...
cc_safe_write_sentinel() (
    umask 077
    cc_sentinel_dir=$(mktemp -d "${TMPDIR:-/tmp}/cc-sentinel-safe-write.XXXXXX" 2>/dev/null) || exit 9
    cleanup_cc_sentinel() {
        rm -rf -- "$cc_sentinel_dir" >/dev/null 2>&1 || return 1
        [ ! -e "$cc_sentinel_dir" ] && [ ! -L "$cc_sentinel_dir" ]
    }
    on_cc_sentinel_signal() {
        trap - 0 HUP INT TERM
        cleanup_cc_sentinel >/dev/null 2>&1 || :
        exit 9
    }
    trap 'cleanup_cc_sentinel >/dev/null 2>&1 || :' 0
    trap on_cc_sentinel_signal HUP INT TERM

    cc_sentinel_work=$cc_sentinel_dir/work
    cc_sentinel_parent=$cc_sentinel_work/parent
    cc_sentinel_parent_link=$cc_sentinel_work/parent-link
    mkdir -m 700 -- "$cc_sentinel_work" "$cc_sentinel_parent" || exit 9
    ln -s "$cc_sentinel_parent" "$cc_sentinel_parent_link" || exit 9
    cc_mutation_parent_stat_follow_valid "$cc_sentinel_parent" || exit $?
    cc_sentinel_parent_identity=$CC_STAT_DEVICE:$CC_STAT_INODE
    cc_sentinel_parent_device=$CC_STAT_DEVICE
    cc_mutation_parent_stat_follow_valid "$cc_sentinel_parent_link" || exit $?
    [ "$CC_STAT_DEVICE:$CC_STAT_INODE" = "$cc_sentinel_parent_identity" ] || exit 1
    case "$CC_STAT_TYPE" in 4???) ;; *) exit 1 ;; esac

    cc_sentinel_tmp=$(cc_mutation_mktemp "$cc_sentinel_work") || exit 9
    printf payload | cc_mutation_stage "$cc_sentinel_tmp" || exit 9
    cc_mutation_stat_valid "$cc_sentinel_tmp" || exit $?
    cc_sentinel_uid=$(id -u) || exit 9
    cc_sentinel_stage_identity=$CC_STAT_DEVICE:$CC_STAT_INODE
    case "$CC_STAT_TYPE" in 8???) ;; *) exit 1 ;; esac
    [ "$CC_STAT_UID" = "$cc_sentinel_uid" ] || exit 1
    [ "$CC_STAT_MODE:$CC_STAT_SIZE:$CC_STAT_LINKS" = 600:7:1 ] || exit 1
    [ "$CC_STAT_DEVICE" = "$cc_sentinel_parent_device" ] || exit 1
    cc_sentinel_hash=$(cc_mutation_hash "$cc_sentinel_tmp") || exit $?
    [ "$cc_sentinel_hash" = 239f59ed55e737c77147cf55ad0c1b030b6d7ee748a7426952f9b852d5a935e5 ] || exit 1

    cc_sentinel_created=$cc_sentinel_work/created
    cc_mutation_link "$cc_sentinel_tmp" "$cc_sentinel_created" || exit 9
    if cc_mutation_link "$cc_sentinel_tmp" "$cc_sentinel_created" 2>/dev/null; then exit 1; fi
    cc_sentinel_link_directory=$cc_sentinel_work/link-directory
    cc_sentinel_directory_link=$cc_sentinel_work/directory-link
    mkdir -m 700 -- "$cc_sentinel_link_directory" || exit 9
    ln -s "$cc_sentinel_link_directory" "$cc_sentinel_directory_link" || exit 9
    if cc_mutation_link "$cc_sentinel_tmp" "$cc_sentinel_directory_link" 2>/dev/null; then exit 1; fi
    cc_sentinel_nested_link=$cc_sentinel_link_directory/${cc_sentinel_tmp##*/}
    [ -L "$cc_sentinel_directory_link" ] || exit 1
    [ ! -e "$cc_sentinel_nested_link" ] && [ ! -L "$cc_sentinel_nested_link" ] || exit 1
    cc_mutation_stat_valid "$cc_sentinel_created" || exit $?
    [ "$CC_STAT_DEVICE:$CC_STAT_INODE" = "$cc_sentinel_stage_identity" ] || exit 1
    cc_mutation_remove "$cc_sentinel_created" || exit 9
    [ ! -e "$cc_sentinel_created" ] && [ ! -L "$cc_sentinel_created" ] || exit 1

    cc_sentinel_replaced=$cc_sentinel_work/replaced
    printf old >"$cc_sentinel_replaced" || exit 9
    cc_mutation_replace "$cc_sentinel_tmp" "$cc_sentinel_replaced" || exit 9
    [ ! -e "$cc_sentinel_tmp" ] && [ ! -L "$cc_sentinel_tmp" ] || exit 1
    cc_mutation_mode 0640 "$cc_sentinel_replaced" || exit 9
    cc_mutation_stat_valid "$cc_sentinel_replaced" || exit $?
    [ "$CC_STAT_MODE" = 640 ] || exit 1
    cc_sentinel_hash=$(cc_mutation_hash "$cc_sentinel_replaced") || exit $?
    [ "$cc_sentinel_hash" = 239f59ed55e737c77147cf55ad0c1b030b6d7ee748a7426952f9b852d5a935e5 ] || exit 1

    cc_sentinel_outside=$cc_sentinel_work/outside
    cc_sentinel_link=$cc_sentinel_work/link
    printf OUTSIDE >"$cc_sentinel_outside" || exit 9
    chmod 0600 -- "$cc_sentinel_outside" || exit 9
    ln -s "$cc_sentinel_outside" "$cc_sentinel_link" || exit 9
    cc_mutation_stat_valid "$cc_sentinel_link" || exit $?
    case "$CC_STAT_TYPE" in a???) ;; *) exit 1 ;; esac
    if printf CHANGED | cc_mutation_stage "$cc_sentinel_link" 2>/dev/null; then exit 1; fi
...
cc_safe_write_preflight() {
    for cc_required_command in stat mktemp dd sha256sum ln mv chmod rm mkdir id cat; do
        command -v "$cc_required_command" >/dev/null 2>&1 || return 1
    done
}

if ! cc_safe_write_preflight; then
    printf 'STATUS=CAPABILITY_MISMATCH\000CAPABILITY=safe_write\000'
    exit 0
fi

cc_sentinel_status=0
cc_safe_write_sentinel || cc_sentinel_status=$?
case "$cc_sentinel_status" in
    0) ;;
    1)
        printf 'STATUS=CAPABILITY_MISMATCH\000CAPABILITY=safe_write\000'
        exit 0
...
cleanup_tmp() {
    [ -z "$tmp" ] && return 0
    cc_mutation_remove "$tmp" >/dev/null 2>&1 || return 1
...
cc_classify_unreachable_parent() {
    cc_parent_candidate=$parent
    cc_parent_unresolved=0
    cc_parent_classification_steps=0
    while :; do
        [ "$cc_parent_classification_steps" -lt 32 ] || return 1
        cc_parent_classification_steps=$((cc_parent_classification_steps + 1))
        cc_parent_lstat_status=0
        cc_mutation_stat_valid "$cc_parent_candidate" || cc_parent_lstat_status=$?
        case "$cc_parent_lstat_status" in
        0)
            case "$CC_STAT_TYPE" in
                4???)
                    if [ ! -x "$cc_parent_candidate" ]; then emit_one PERMISSION_DENIED; fi
                    [ "$cc_parent_unresolved" -gt 0 ] && emit_one NOT_FOUND
                    return 1
                    ;;
                a???)
                    cc_parent_follow_status=0
                    cc_mutation_parent_stat_follow_valid "$cc_parent_candidate" || cc_parent_follow_status=$?
                    [ "$cc_parent_follow_status" -eq 0 ] || return 1
                    case "$CC_STAT_TYPE" in
                        4???)
                            if [ ! -x "$cc_parent_candidate" ]; then emit_one PERMISSION_DENIED; fi
                            [ "$cc_parent_unresolved" -gt 0 ] && emit_one NOT_FOUND
                            return 1
                            ;;
                        *) [ "$cc_parent_unresolved" -gt 0 ] && emit_one NOT_DIRECTORY; return 1 ;;
                    esac
                    ;;
                *) [ "$cc_parent_unresolved" -gt 0 ] && emit_one NOT_DIRECTORY; return 1 ;;
            esac
        ...
        *) return 1 ;;
        ...
        [ "$cc_parent_candidate" = . ] && return 1
        cc_parent_candidate=${cc_parent_candidate%/*}
        [ -n "$cc_parent_candidate" ] || cc_parent_candidate=.
        cc_parent_unresolved=$((cc_parent_unresolved + 1))
    done
}

cc_parent_line=$(cc_mutation_parent_stat_follow "$parent") || {
    cc_classify_unreachable_parent
    exit 3
}
cc_mutation_stat_parse "$cc_parent_line" || exit 3
case "$CC_STAT_TYPE" in 4???) ;; *) emit_one NOT_DIRECTORY ;; esac
parent_device=$CC_STAT_DEVICE
parent_inode=$CC_STAT_INODE

if ! CDPATH= cd -P -- "$parent" 2>/dev/null; then
    if cc_mutation_parent_stat_follow_valid "$parent" &&
       [ "$CC_STAT_DEVICE:$CC_STAT_INODE" = "$parent_device:$parent_inode" ]; then
        case "$CC_STAT_TYPE" in 4???) emit_one PERMISSION_DENIED ;; esac
    fi
    exit 3
fi
cc_mutation_parent_stat_follow_valid . || exit 3
[ "$CC_STAT_DEVICE:$CC_STAT_INODE" = "$parent_device:$parent_inode" ] || exit 3

target=./$basename
target_initial_status=0
cc_mutation_stat_valid "$target" || target_initial_status=$?
if [ "$target_initial_status" -eq 0 ]; then
    if [ "$operation" = CREATE ]; then
        emit_one WRITE_CONFLICT
    fi
    case "$CC_STAT_TYPE" in 8???) ;; *) emit_one WRITE_CONFLICT ;; esac
    target_device=$CC_STAT_DEVICE
    target_inode=$CC_STAT_INODE
    target_mode=$CC_STAT_MODE
...
umask 077
tmp=$(cc_mutation_mktemp .) || exit 4
cc_mutation_stage "$tmp" || exit 4
cc_mutation_stat_valid "$tmp" || exit 4
stage_device=$CC_STAT_DEVICE
stage_inode=$CC_STAT_INODE
stage_uid=$(id -u) || exit 4
case "$CC_STAT_TYPE" in 8???) ;; *) exit 4 ;; esac
[ "$CC_STAT_UID" = "$stage_uid" ] || exit 4
[ "$CC_STAT_MODE" = 600 ] || exit 4
[ "$CC_STAT_SIZE" = "$expected_size" ] || exit 4
[ "$CC_STAT_DEVICE" = "$parent_device" ] || exit 4
[ "$CC_STAT_LINKS" = 1 ] || exit 4
stage_hash=$(cc_mutation_hash "$tmp") || exit 4
[ "$stage_hash" = "$expected_content_hash" ] || exit 4

if [ "$operation" = CREATE ]; then
    if ! cc_mutation_link "$tmp" "$target" 2>/dev/null; then
        if cc_mutation_stat_valid "$target" || [ -e "$target" ] || [ -L "$target" ]; then
            emit_one WRITE_CONFLICT
        fi
        exit 5
    fi
    ...
    [ "$CC_STAT_UID:$CC_STAT_MODE:$CC_STAT_SIZE:$CC_STAT_LINKS" = "$stage_uid:600:$expected_size:2" ] || exit 5
    target_hash=$(cc_mutation_hash "$target") || exit 5
    [ "$target_hash" = "$expected_content_hash" ] || exit 5

    cc_mutation_remove "$tmp" || exit 5
    [ ! -e "$tmp" ] && [ ! -L "$tmp" ] || exit 5
    tmp=
...
cc_mutation_stat_valid "$target" || target_final_status=$?
...
case "$CC_STAT_TYPE" in 8???) ;; *) emit_one WRITE_CONFLICT ;; esac
[ "$CC_STAT_DEVICE:$CC_STAT_INODE:$CC_STAT_MODE" = "$target_device:$target_inode:$target_mode" ] || emit_one WRITE_CONFLICT
target_final_mode_decimal=$((0$CC_STAT_MODE))
[ $((target_final_mode_decimal & 07000)) -eq 0 ] || emit_one WRITE_CONFLICT

if [ "$expected_hash_present" = 1 ]; then
    target_hash_status=0
    target_hash=$(cc_mutation_hash "$target") || target_hash_status=$?
    if [ "$target_hash_status" -ne 0 ]; then
        if cc_mutation_stat_valid "$target"; then
            case "$CC_STAT_TYPE" in 8???) ;; *) emit_one WRITE_CONFLICT ;; esac
            [ "$CC_STAT_DEVICE:$CC_STAT_INODE:$CC_STAT_MODE" = "$target_device:$target_inode:$target_mode" ] || emit_one WRITE_CONFLICT
        else
            exit 5
...
cc_guarded_delete_sentinel() (
    umask 077
    cc_sentinel_dir=$(mktemp -d "${TMPDIR:-/tmp}/cc-sentinel-guarded-delete.XXXXXX" 2>/dev/null) || exit 9
    cleanup_cc_sentinel() {
        rm -rf -- "$cc_sentinel_dir" >/dev/null 2>&1 || return 1
        [ ! -e "$cc_sentinel_dir" ] && [ ! -L "$cc_sentinel_dir" ]
    }
    ...
    cc_sentinel_parent=$cc_sentinel_dir/parent
    cc_sentinel_parent_link=$cc_sentinel_dir/parent-link
    mkdir -m 700 -- "$cc_sentinel_parent" || exit 9
    ln -s "$cc_sentinel_parent" "$cc_sentinel_parent_link" || exit 9
    cc_mutation_parent_stat_follow_valid "$cc_sentinel_parent" || exit $?
    cc_sentinel_parent_identity=$CC_STAT_DEVICE:$CC_STAT_INODE
    cc_mutation_parent_stat_follow_valid "$cc_sentinel_parent_link" || exit $?
    ...
    cc_sentinel_victim=$cc_sentinel_parent/victim
    printf payload >"$cc_sentinel_victim" || exit 9
    cc_mutation_stat_valid "$cc_sentinel_victim" || exit $?
    case "$CC_STAT_TYPE" in 8???) ;; *) exit 1 ;; esac
    [ "$CC_STAT_MODE:$CC_STAT_SIZE:$CC_STAT_LINKS" = 600:7:1 ] || exit 1
    cc_sentinel_identity=$CC_STAT_DEVICE:$CC_STAT_INODE
    cc_sentinel_hash=$(cc_mutation_hash "$cc_sentinel_victim") || exit $?
    [ "$cc_sentinel_hash" = 239f59ed55e737c77147cf55ad0c1b030b6d7ee748a7426952f9b852d5a935e5 ] || exit 1
    cc_mutation_stat_valid "$cc_sentinel_victim" || exit $?
    case "$CC_STAT_TYPE" in 8???) ;; *) exit 1 ;; esac
    [ "$CC_STAT_DEVICE:$CC_STAT_INODE" = "$cc_sentinel_identity" ] || exit 1
    cc_sentinel_hash=$(cc_mutation_hash "$cc_sentinel_victim") || exit $?
    [ "$cc_sentinel_hash" = 239f59ed55e737c77147cf55ad0c1b030b6d7ee748a7426952f9b852d5a935e5 ] || exit 1
    cc_mutation_remove "$cc_sentinel_victim" || exit 9
    [ ! -e "$cc_sentinel_victim" ] && [ ! -L "$cc_sentinel_victim" ] || exit 1

    cc_sentinel_outside=$cc_sentinel_dir/outside
    cc_sentinel_link=$cc_sentinel_parent/link
    printf OUTSIDE >"$cc_sentinel_outside" || exit 9
    ln -s "$cc_sentinel_outside" "$cc_sentinel_link" || exit 9
    cc_mutation_stat_valid "$cc_sentinel_link" || exit $?
    case "$CC_STAT_TYPE" in a???) ;; *) exit 1 ;; esac
...
cc_guarded_delete_preflight() {
    for cc_required_command in stat mktemp dd sha256sum ln rm mkdir cat; do
        command -v "$cc_required_command" >/dev/null 2>&1 || return 1
    done
}

if ! cc_guarded_delete_preflight; then
    printf 'STATUS=CAPABILITY_MISMATCH\000CAPABILITY=guarded_delete\000'
    exit 0
fi

cc_sentinel_status=0
cc_guarded_delete_sentinel || cc_sentinel_status=$?
case "$cc_sentinel_status" in
    0) ;;
    1)
        printf 'STATUS=CAPABILITY_MISMATCH\000CAPABILITY=guarded_delete\000'
...
target=./$basename
target_status=0
cc_mutation_stat_valid "$target" || target_status=$?
...
if [ "$target_hash_status" -ne 0 ]; then
    if cc_mutation_stat_valid "$target"; then
        case "$CC_STAT_TYPE" in 8???) ;; *) emit_one WRITE_CONFLICT ;; esac
        [ "$CC_STAT_DEVICE:$CC_STAT_INODE" = "$target_device:$target_inode" ] || emit_one WRITE_CONFLICT
    else
...
cc_mutation_remove "$target" || exit 5
...
mod tests {
    ...
    fn assert_no_dispatcher_request_artifacts(directory: &std::path::Path) {
        let unexpected = std::fs::read_dir(directory)
            ...
            .filter(|name| {
                !name
                    .to_string_lossy()
                    .starts_with("cc-ssh-bridge-dispatcher.")
            })
            .collect::<Vec<_>>();
        assert!(
            unexpected.is_empty(),
    ...
    fn task5_shell_stat_parsers_declare_closed_numeric_shapes() {
        for script in [super::WRITE_SCRIPT, super::GUARDED_DELETE_SCRIPT] {
            ...
            assert!(script.contains("cc_mutation_decimal_valid \"$2\" || return 1"));
            assert!(script.contains("cc_mutation_decimal_valid \"$4\" || return 1"));
            assert!(script.contains("cc_mutation_decimal_valid \"$5\" || return 1"));
    ...
    async fn task5_guarded_delete_sentinel_closes_second_stat_and_hash_drift() {
        for (tool, expected_code) in [
            ...
            let body = if tool == "stat" {
                format!(
                    "#!/bin/sh\ncase \" $* \" in *cc-sentinel-guarded-delete*/parent/victim*) marker={}; count=$(/usr/bin/cat \"$marker\" 2>/dev/null || printf 0); count=$((count + 1)); if [ \"$count\" -eq 2 ]; then /usr/bin/rm -f \"$marker\"; printf 'a1ff:0:777:7:1:2:1\\n'; exit 0; fi; printf %s \"$count\" >\"$marker\";; esac\nexec /usr/bin/stat \"$@\"\n",
                    crate::quote::shell_word(marker.to_str().unwrap()).unwrap()
                )
            } else {
                format!(
                    "#!/bin/sh\ncase \" $* \" in *cc-sentinel-guarded-delete*/parent/victim*bs=262144*iflag=nofollow*) marker={}; count=$(/usr/bin/cat \"$marker\" 2>/dev/null || printf 0); count=$((count + 1)); if [ \"$count\" -eq 2 ]; then /usr/bin/rm -f \"$marker\"; exit 64; fi; printf %s \"$count\" >\"$marker\";; esac\nexec /usr/bin/dd \"$@\"\n",
                    crate::quote::shell_word(marker.to_str().unwrap()).unwrap()
                )
            };
    ...
    async fn task5_guarded_delete_stale_sentinel_reprobes_only_the_next_call() {
        ...
        std::fs::write(
            &stat,
            format!(
                "#!/bin/sh\ncase \" $* \" in *cc-sentinel-guarded-delete*parent-link*) marker={}; if [ ! -e \"$marker\" ]; then : >\"$marker\"; printf '41c0:0:700:0:1:2:1:extra\\n'; exit 0; fi;; esac\nexec /usr/bin/stat \"$@\"\n",
                crate::quote::shell_word(marker.to_str().unwrap()).unwrap()
            ),
        )
        .unwrap();
    ...
    async fn task5_delete_exact_form_sentinel_matrix_is_semantic_and_future_only() {
        ...
        let cases = [
            Case {
                ...
                tool: "stat",
                rule: r#"case " $* " in
  *" -L --printf=%f:%u:%a:%s:%d:%i:%h\n -- "*cc-sentinel-guarded-delete*/parent-link*)
    if [ ! -e "$marker" ]; then : >"$marker"; printf '41ed:0:700:0:0:0:2\n'; exit 0; fi;;
            ...
            Case {
                ...
                tool: "stat",
                rule: r#"case " $* " in
  *" --printf=%f:%u:%a:%s:%d:%i:%h\n -- "*cc-sentinel-guarded-delete*/parent/victim*)
    if [ ! -e "$marker" ]; then : >"$marker"; printf '8180:0:640:8:0:0:1\n'; exit 0; fi;;
            ...
            Case {
                ...
                tool: "dd",
                rule: r#"case " $* " in
  *cc-sentinel-guarded-delete*/parent/victim*bs=262144*status=none*iflag=nofollow*)
  ...
  exit 0
            ...
            Case {
                ...
                tool: "rm",
                rule: r#"case " $* " in
  *" -f -- "*cc-sentinel-guarded-delete*/parent/victim*)
        ...
        for case in cases {
            ...
            if case.tool == "sha256sum" {
                ...
                std::fs::write(
                    &stat,
                    format!(
                        "#!/bin/sh\ncase \" $* \" in *\" --printf=%f:%u:%a:%s:%d:%i:%h\\n -- \"*cc-sentinel-guarded-delete*/parent/victim*) : >{};; esac\nexec /usr/bin/stat \"$@\"\n",
                        crate::quote::shell_word(armed.to_str().unwrap()).unwrap(),
                    ),
                )
                .unwrap();

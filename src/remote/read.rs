...
budget=$4
cc_check_read() (
    cc_read_dir=$(mktemp -d /tmp/cc-sentinel-read.XXXXXX 2>/dev/null) || exit 9
    cleanup_cc_read() { rm -rf -- "$cc_read_dir"; }
    trap cleanup_cc_read EXIT HUP INT TERM
    cc_read_file=$cc_read_dir/cc-sentinel-read
    cc_read_tail=$cc_read_dir/tail
    cc_read_line=$cc_read_dir/line
    cc_read_bytes=$cc_read_dir/bytes
    cc_read_last=$cc_read_dir/last
    cc_read_expected=$cc_read_dir/expected
    printf 'a\000b\nc' >"$cc_read_file" || exit 9
    printf 'a\000b\n' >"$cc_read_expected" || exit 9
    tail -n +1 -- "$cc_read_file" >"$cc_read_tail" 2>/dev/null || exit 1
    head -n 1 "$cc_read_tail" >"$cc_read_line" 2>/dev/null || exit 1
    head -c 4 "$cc_read_line" >"$cc_read_bytes" 2>/dev/null || exit 1
    tail -c 1 -- "$cc_read_file" >"$cc_read_last" 2>/dev/null || exit 1
    cc_read_count=$(wc -l <"$cc_read_file") || exit 1
    cc_read_size=$(stat --printf='%s' -- "$cc_read_file" 2>/dev/null) || exit 2
    cc_read_hash=$(sha256sum -- "$cc_read_file" 2>/dev/null) || exit 3
    set -- $cc_read_hash
    cc_read_hash=$1
    cmp -s "$cc_read_expected" "$cc_read_bytes" &&
    [ "$(cat "$cc_read_last")" = c ] && [ "$cc_read_count" -eq 1 ] || exit 1
    [ "$cc_read_size" -eq 5 ] || exit 2
    [ "$cc_read_hash" = 214df3f68e1a607f5baa40cc3315f4316ae58b282b6c0bf288b89fec4da7aa80 ] || exit 3
)
cc_read_status=0
cc_check_read || cc_read_status=$?
case "$cc_read_status" in
    0) ;;

...
pub(super) fn parse_codex_patch(input: &str) -> BridgeResult<Vec<CodexFilePatch>> {
    ...
    if records.len() > MAX_CODEX_RECORDS {
        return Err(patch_too_large(
            "apply_patch envelope contains too many records",
        ));
    }
    if records.first().copied() != Some(BEGIN_PATCH) {
        return Err(invalid_patch("apply_patch begin marker is missing"));
    }
    let end = records
        .iter()
        .position(|record| *record == END_PATCH)
        .ok_or_else(|| invalid_patch("apply_patch end marker is missing"))?;
    if end + 1 != records.len() {
        return Err(invalid_patch("apply_patch envelope has trailing data"));
    }
    ...
    while index < end {
        ...
        if record.starts_with(MOVE_TO) {
            return Err(invalid_patch("apply_patch move is unsupported"));
        }
        let (operation, path) = if let Some(path) = record.strip_prefix(ADD_FILE) {
            ...
            return Err(invalid_patch("apply_patch file directive is invalid"));
        };
        ...
        let patch = match operation {
            FilePatchOperation::Create => {
                ...
                while index < end && !is_file_directive(records[index]) {
                    ...
                    let text = record
                        .strip_prefix('+')
                        .ok_or_else(|| invalid_patch("apply_patch add-file line is invalid"))?;
                    increment_body_lines(&mut total_body_lines)?;
                    extend_line(&mut bytes, text, MAX_PATCH_BYTES)?;
                    index += 1;
                }
                if bytes.is_empty() {
                    return Err(invalid_patch("apply_patch add-file section is empty"));
                }
                CodexFilePatch {
                    path: path.to_owned(),
                    operation,
            ...
            FilePatchOperation::Update => {
                ...
                if chunks.is_empty() {
                    return Err(invalid_patch("apply_patch update-file section is empty"));
                }
                CodexFilePatch {
                    path: path.to_owned(),
                    operation,
    ...
    if patches.is_empty() {
        return Err(invalid_patch(
            "apply_patch envelope contains no file operations",
        ));
    }
    Ok(patches)
}
...
fn parse_update_chunk(
    ...
    let context = if records[index] == "@@" {
        ...
        if context.is_empty() {
            return Err(invalid_patch("apply_patch context is empty"));
        }
        index += 1;
    ...
    while index < end {
        ...
        let (prefix, text) = record
            .split_at_checked(1)
            .ok_or_else(|| invalid_patch("apply_patch update-file line is invalid"))?;
        match prefix.as_bytes()[0] {
            ...
            _ => return Err(invalid_patch("apply_patch update-file line is invalid")),
        }
        increment_body_lines(total_body_lines)?;
        index += 1;
    }
    if !changed {
        return Err(invalid_patch("apply_patch update chunk has no changes"));
    }
    if end_of_file && index < end {
        ...
        if !is_file_directive(records[index]) {
            return Err(invalid_patch("apply_patch end-of-file marker is not final"));
        }
    }
    Ok((
        CodexUpdateChunk {
...
fn validate_codex_path(path: &str) -> BridgeResult<()> {
    if !path.starts_with('/') {
        return Err(invalid_patch("apply_patch path is not absolute"));
    }
    if path.contains(['\t', '\r', '\n']) {
        return Err(invalid_patch("apply_patch path is invalid"));
    }
...
fn reject_nested_or_mixed_record(record: &str) -> BridgeResult<()> {
    if record == BEGIN_PATCH || record == END_PATCH {
        return Err(invalid_patch("apply_patch envelope is nested"));
    }
    if record.starts_with("--- ") || record.starts_with("+++ ") {
        return Err(invalid_patch(
            "apply_patch envelope contains unified diff syntax",
        ));
    }
    if record.starts_with("*** ") {
        return Err(invalid_patch("apply_patch marker is invalid"));
    }
    Ok(())
}
    ...
    match patch.operation {
        FilePatchOperation::Create => {
            ...
            let bytes = patch
                .add_bytes
                .as_ref()
                .ok_or_else(|| invalid_patch("apply_patch add-file content is missing"))?;
    ...
    for chunk in &patch.chunks {
        if let Some(context) = &chunk.context {
            ...
            let found = find_sequence(&lines, context, cursor, false)
                .ok_or_else(|| write_conflict("apply_patch context was not found"))?;
            cursor = found + 1;
        }
        let start = find_sequence(&lines, &chunk.old_lines, cursor, chunk.end_of_file)
            .ok_or_else(|| write_conflict("apply_patch expected lines were not found"))?;

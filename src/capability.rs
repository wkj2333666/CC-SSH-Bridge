...
if [ "$tool_mktemp" = 1 ]; then
    probe_tmp=$(mktemp -d "${TMPDIR:-/tmp}/cc-ssh-bridge.XXXXXX") || {
        tool_mktemp=0
        probe_tmp=
    }
fi
...
if [ -n "$probe_tmp" ]; then
    read_file=$probe_tmp/cc-probe-read
    ...
    find_actual=$probe_tmp/cc-probe-find
    find_link=$probe_tmp/cc-probe-find-link
...
name'
    ...
    if (cd "$probe_tmp" &&
        find -H cc-probe-find-link -mindepth 1 -maxdepth 2 \
    ...
    stat_file=$probe_tmp/cc-probe-stat
    printf x >"$stat_file"
    ...
    rg_file=$probe_tmp/cc-probe-rg-text
    rg_bytes_file=$probe_tmp/cc-probe-rg-bytes
    rg_binary_file=$probe_tmp/cc-probe-rg-binary
    rg_error_file=$probe_tmp/cc-probe-rg-error
    ...
    grep_file=$probe_tmp/cc-probe-grep
    ...
    printf 'line\nname\000' |
        xargs -0 -r sh -c 'printf %s "$1"' cc-ssh-probe-xargs >"$xargs_out" 2>/dev/null
    xargs_ok=$?
    printf 'x\000' |
        xargs -0 -r sh -c 'exit 7' cc-ssh-probe-xargs >/dev/null 2>&1
    xargs_failure=$?
    ...
    if command -v mkfifo >/dev/null 2>&1; then
        bound_dir=$(mktemp -d "$probe_tmp/cc-probe-bound.XXXXXX" 2>/dev/null) || bound_dir=
        if [ -n "$bound_dir" ] && (
            ...
            (
                printf 'ab\000cd\000' |
                    xargs -0 -r -n 1 sh -c 'printf %s "$1"' cc-ssh-probe-bound-xargs >"$bound_fifo" 2>/dev/null
            ...
            (
                printf 'abcdef\000' |
                    xargs -0 -r sh -c 'printf %s "$1"; exit 7' cc-ssh-probe-bound-xargs >"$bound_error_fifo" 2>/dev/null
    ...
    safe_dir=$probe_tmp/cc-probe-safe-write
    if (
        umask 077
...
name' value"
        ...
        cc_mutation_parent_stat_follow_valid "$safe_parent_actual" || exit 1
        safe_follow_device=$CC_STAT_DEVICE
        safe_follow_inode=$CC_STAT_INODE
        cc_mutation_parent_stat_follow_valid "$safe_parent_link" || exit 1
        [ "$CC_STAT_DEVICE:$CC_STAT_INODE" = "$safe_follow_device:$safe_follow_inode" ] || exit 1
        case "$CC_STAT_TYPE" in 4???) ;; *) exit 1 ;; esac

        cc_mutation_parent_stat_follow_valid "$safe_dir" || exit 1
        safe_parent_device=$CC_STAT_DEVICE
        safe_tmp=$(cc_mutation_mktemp "$safe_dir") || exit 1
        printf payload | cc_mutation_stage "$safe_tmp" || exit 1
        cc_mutation_stat_valid "$safe_tmp" || exit 1
        safe_uid=$(id -u) || exit 1
        case "$CC_STAT_TYPE" in 8???) ;; *) exit 1 ;; esac
        [ "$CC_STAT_UID" = "$safe_uid" ] &&
        [ "$CC_STAT_MODE" = 600 ] &&
        [ "$CC_STAT_SIZE" = 7 ] &&
        [ "$CC_STAT_DEVICE" = "$safe_parent_device" ] &&
        [ "$CC_STAT_LINKS" = 1 ] || exit 1
        CC_HASH_DIGEST=$(cc_mutation_hash "$safe_tmp") || exit 1
        [ "$CC_HASH_DIGEST" = 239f59ed55e737c77147cf55ad0c1b030b6d7ee748a7426952f9b852d5a935e5 ] || exit 1

        printf OUTSIDE >"$safe_hostile" || exit 9
        ln -s "$safe_hostile" "$safe_dir/dd-link" || exit 9
        cc_mutation_stat_valid "$safe_dir/dd-link" || exit 1
        case "$CC_STAT_TYPE" in a???) ;; *) exit 1 ;; esac
        if printf CHANGED | cc_mutation_stage "$safe_dir/dd-link" 2>/dev/null; then exit 1; fi
        [ "$(cat "$safe_hostile")" = OUTSIDE ] || exit 1
        if CC_HASH_DIGEST=$(cc_mutation_hash "$safe_dir/dd-link"); then exit 1; fi

        safe_created=$safe_dir/created
        cc_mutation_link "$safe_tmp" "$safe_created" || exit 1
        if cc_mutation_link "$safe_tmp" "$safe_created" 2>/dev/null; then exit 1; fi
        ...
        if cc_mutation_link "$safe_tmp" "$safe_directory_link" 2>/dev/null; then exit 1; fi
        safe_nested_link=$safe_link_directory/${safe_tmp##*/}
        [ -L "$safe_directory_link" ] || exit 1
        [ ! -e "$safe_nested_link" ] && [ ! -L "$safe_nested_link" ] || exit 1
        cc_mutation_remove "$safe_created" || exit 1
        [ ! -e "$safe_created" ] && [ ! -L "$safe_created" ] || exit 1
        safe_replaced=$safe_dir/replaced
        printf old >"$safe_replaced" || exit 9
        cc_mutation_replace "$safe_tmp" "$safe_replaced" || exit 1
        [ ! -e "$safe_tmp" ] && [ ! -L "$safe_tmp" ] || exit 1
        cc_mutation_mode 0640 "$safe_replaced" || exit 1
        cc_mutation_stat_valid "$safe_replaced" || exit 1
        [ "$CC_STAT_MODE" = 640 ] || exit 1
        CC_HASH_DIGEST=$(cc_mutation_hash "$safe_replaced") || exit 1
        [ "$CC_HASH_DIGEST" = 239f59ed55e737c77147cf55ad0c1b030b6d7ee748a7426952f9b852d5a935e5 ] || exit 1
        ...
        cc_mutation_mode 0640 "$safe_link" || exit 1
    ...
    delete_dir=$probe_tmp/cc-probe-guarded-delete
    if (
        umask 077
...
name' value"
        cc_mutation_parent_stat_follow_valid "$delete_dir" || exit 1
        case "$CC_STAT_TYPE" in 4???) ;; *) exit 1 ;; esac
        printf guarded >"$delete_target" || exit 9
        cc_mutation_stat_valid "$delete_target" || exit 1
        case "$CC_STAT_TYPE" in 8???) ;; *) exit 1 ;; esac
        delete_device=$CC_STAT_DEVICE
        delete_inode=$CC_STAT_INODE
        CC_HASH_DIGEST=$(cc_mutation_hash "$delete_target") || exit 1
        [ "$CC_HASH_DIGEST" = e7da2135b6d3a82d5242bbb5d7b8534e5841356c6973e1d479ce41e41dd6215b ] || exit 1
        cc_mutation_stat_valid "$delete_target" || exit 1
        case "$CC_STAT_TYPE" in 8???) ;; *) exit 1 ;; esac
        [ "$CC_STAT_DEVICE:$CC_STAT_INODE" = "$delete_device:$delete_inode" ] || exit 1
        CC_HASH_DIGEST=$(cc_mutation_hash "$delete_target") || exit 1
        [ "$CC_HASH_DIGEST" = e7da2135b6d3a82d5242bbb5d7b8534e5841356c6973e1d479ce41e41dd6215b ] || exit 1
        cc_mutation_remove "$delete_target" || exit 1
        ...
        cc_mutation_stat_valid "$delete_dir/link" || exit 1
        case "$CC_STAT_TYPE" in a???) ;; *) exit 1 ;; esac
...
pub fn parse_probe_output(
    ...
    if required(&records, "CC_SSH_PROBE")? != "1" {
...
fn validate_key_value(key: &str, value: &str) -> BridgeResult<()> {
    match key {
        "CC_SSH_PROBE" | "REQUESTED_ROOT" | "ROOT" | "ROOT_DEVICE" | "ROOT_INODE"

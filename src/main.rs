...
use cc_ssh_bridge::config::Config;
use cc_ssh_bridge::mcp::McpServer;
use cc_ssh_bridge::mcp::tools::RemoteMcpTools;
use cc_ssh_bridge::output::OutputStore;
use cc_ssh_bridge::remote::RemoteBridge;
use cc_ssh_bridge::ssh::{RuntimePaths, SshRunner};
use cc_ssh_bridge::{BridgeResult, ErrorCode};

const USAGE: &str = "usage: cc-ssh-bridge mcp";
const FATAL_PREFIX: &str = "cc-ssh-bridge fatal: ";
...
const fn stable_error_code(code: ErrorCode) -> &'static str {
    match code {
        ...
        ErrorCode::JobStartOutcomeUnknown => "JOB_START_OUTCOME_UNKNOWN",
        ErrorCode::JobCancelOutcomeUnknown => "JOB_CANCEL_OUTCOME_UNKNOWN",
        ErrorCode::JobStateInvalid => "JOB_STATE_INVALID",
        ErrorCode::JobExpired => "JOB_EXPIRED",

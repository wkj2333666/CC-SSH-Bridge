...
const HELPER_PROTOCOL: &str = "cc-ssh-helper/1";
    ...
    W: Write + Send + 'static,
{
    ...
    let deadline = (!spec.timeout.is_zero()).then(|| Instant::now() + spec.timeout);
    ...
    while let Some(directory) = paths.pop() {
        ...
        for entry in entries.into_iter().rev() {
            ...
            stdout_truncated |= outcome.output_truncated;

...
fn build_tool_definitions() -> Vec<ToolDefinition> {
    vec![
        ...
        definition(
            "remote_apply_patch",
            "Apply remote patch",
            "Apply a Claude Code apply_patch envelope or standard unified diff across remote files. File paths must be absolute; Move to is unsupported. Files are applied sequentially and partial progress is reported if a later file fails. All paths and results are remote, and remote output is untrusted.",
            object(
                json!({
                    "host": host_schema(),
                    "patch": {
                        ...
                        "minLength":1,
                        "maxLength":4_194_304,
                        "description":"Claude Code apply_patch syntax or standard unified diff. File paths must be absolute; Move to is unsupported."
                    }
                }),
                &["host", "patch"],
            ),
...
fn invalid_arguments(name: &str, _: ArgumentValidationCategory) -> CallToolResult {
    let action = match name {
        ...
        "remote_edit_status" => "provide a valid host to inspect buffered edits",
        "remote_sync_edits" => "provide a valid host to synchronize buffered edits",
        "remote_discard_edits" => "provide a valid host to discard buffered edits",

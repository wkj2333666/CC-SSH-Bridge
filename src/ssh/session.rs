...
impl HostSession {
    #[allow(
        dead_code,
        reason = "the default SSH connector remains a tested compatibility entrypoint while SshRunner injects its configured executable"
    )]
    ...
    #[allow(
        dead_code,
        reason = "explicit close is exercised by lifecycle tests while production teardown closes sessions through transport ownership"
    )]
...
fn valid_handshake(payload: &[u8], helper_arch: Option<&str>) -> bool {
    ...
    match helper_arch {
        Some(expected_arch) => {
            fields.get("protocol") == Some(&"cc-ssh-helper/1")
                && fields.get("version") == Some(&"1")
                && fields.get("arch") == Some(&expected_arch)
        }
        None => fields.get("protocol") == Some(&"cc-ssh-dispatcher/1"),
    }
}
...
mod tests {
    ...
    use super::{
        ConnectionRuntime, ConnectionStart, HelperArtifact, HostSession, Outbound, SessionAction,
        SessionInner, SessionOutput, SessionRequest, parse_exit, valid_handshake,
    };
    ...
    fn helper_and_shell_handshakes_are_checked_against_the_selected_transport() {
        assert!(valid_handshake(
            b"protocol=cc-ssh-helper/1;version=1;arch=x86_64;",
            Some("x86_64")
        ));
        assert!(!valid_handshake(
            b"protocol=cc-ssh-helper/1;version=1;arch=aarch64;",
            Some("x86_64")
        ));
        assert!(!valid_handshake(
            b"protocol=cc-ssh-dispatcher/1;shell=sh;",
            Some("x86_64")
        ));
        assert!(valid_handshake(
            b"protocol=cc-ssh-dispatcher/1;shell=sh;",
    ...
    fn policy() -> SshPolicy {
        SshPolicy {
            options: Vec::new(),
            control_path: PathBuf::from("/tmp/cc-ssh-session-test-control"),
        }
    }

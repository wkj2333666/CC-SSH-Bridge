...
mod tests {
    ...
    fn supported_architectures_map_to_static_helper_targets() {
        for (arch, target) in [
            ...
            if std::env::var_os("CC_SSH_BRIDGE_HELPERS_DIR").is_some() {
    ...
    fn bootstrap_is_shell_quoted_and_carries_exact_lengths() {
        ...
        assert!(command.contains("cc-ssh-helper-bootstrap-1"));
    ...
    fn bootstrap_uploads_and_executes_the_helper_without_interpreting_bytes() {
        ...
        let helper_path = std::env::var("CARGO_BIN_EXE_cc-ssh-bridge-helper")

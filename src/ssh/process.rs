...
use super::{
    HelperMode, HostSession, RuntimePaths, SessionRequest, SessionResult, SshPolicy,
...
#[allow(
    dead_code,
    reason = "one-shot command phases remain for transport parity while initialization still uses resolve and probe"
)]
enum Phase {
    Resolve,
    Probe,
...
exec sh -c "$2""#;
    ...
    const ARG0: &str = " cc-ssh-bridge-run ";
    const SEPARATOR: &str = " ";
...
const FIXED_COMMAND_ARG0: &str = " cc-ssh-bridge-op";
...
mod tests {
    ...
    fn remote_timeout_uses_gnu_decimal_seconds() {
        ...
        assert!(command.contains(" cc-ssh-bridge-run '/' 'exit 0' '0.123s'"));
        assert_eq!(
    ...
    fn run_quote_admission_release_rss() {
        const CHILD_ENV: &str = "CC_SSH_BRIDGE_QUOTE_RSS_CHILD";
    ...
    fn direct_rendering_has_no_physical_root_protocol() {
        ...
        assert!(!fixed.contains("CC_SSH_ROOT_OBSERVE"));
        ...
        .unwrap();
        assert!(!run.contains("CC_SSH_ROOT_OBSERVE"));

...
exit 0"
    ...
    ' cc-ssh-dispatcher "$run_cwd" "$run_shell" "$run_command" "$run_dir/pid" "$run_command_path" "$run_login_shell" \
    ...
    if [ -e "$run_dir/cancelled" ]; then
        ...
        setsid sh -c 'sleep 0.05; kill -KILL -"$1" 2>/dev/null || true' cc-ssh-killer "$run_pid" &
    fi
    if [ -n "${CC_SSH_BRIDGE_TEST_MODE-}" ] &&

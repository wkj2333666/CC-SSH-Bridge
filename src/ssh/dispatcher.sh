...
umask 077
PROTOCOL_TAG=${1-}
[ "$PROTOCOL_TAG" = cc-ssh-dispatcher-1 ] || exit 64
...
BASE=$BASE_ROOT/cc-ssh-bridge-dispatcher.$$
base_suffix=0
while ! mkdir "$BASE" 2>/dev/null; do
    base_suffix=$((base_suffix + 1))
    [ "$base_suffix" -le 100 ] || exit 73
    BASE=$BASE_ROOT/cc-ssh-bridge-dispatcher.$$.$base_suffix
done
    ...
    if [ -f "$request_dir/pid" ]; then
        ...
        case "$request_pid" in
            ...
            *)
                ...
                setsid sh -c 'sleep 0.05; kill -KILL -"$1" 2>/dev/null || true' cc-ssh-killer "$request_pid" &
                ;;
        esac
    fi
    rm -rf "$request_dir"
...
record_test_call() {
    if [ -n "${CC_SSH_BRIDGE_TEST_CALL_LOG-}" ]; then
        record_kind=$1
        record_path=$2
        record_log=$CC_SSH_BRIDGE_TEST_CALL_LOG
...
record_test_phase() {
    if [ -n "${FAKE_SSH_PHASE_LOG-}" ]; then
        case "$(cat "$1")" in
            ...
            *cc_safe_write_sentinel*|*cc_guarded_delete_sentinel*) printf 'M\n' >>"$FAKE_SSH_PHASE_LOG" ;;
        esac
    fi
}
...
run_request() {
    ...
    run_is_root=0
    case "$run_command" in *CC_SSH_ROOT_OBSERVE*) run_is_root=1 ;; esac
    ...
    case "$run_command" in
        *CC_SSH_ROOT_OBSERVE*) ;;
        *)
            if [ -n "${CC_SSH_BRIDGE_TEST_MODE-}" ]; then
    ...
    if [ -n "${CC_SSH_BRIDGE_TEST_MODE-}" ] &&
       [ "${FAKE_SSH_MODE-}" != local-fixed ]; then
        case "$run_command" in
            *CC_SSH_ROOT_OBSERVE*)
        run_command='printf "CC_SSH_ROOT_OBSERVE=1\\000ROOT=%s\\000DEVICE=%s\\000INODE=%s\\000" "${FAKE_SSH_ROOT:-/srv/project}" "${FAKE_SSH_ROOT_DEVICE:-1}" "${FAKE_SSH_ROOT_INODE:-1}"'
                ;;
        esac
    fi
    if [ -n "${CC_SSH_LOCAL_FIXED_PATH_ONCE-}" ] &&
       [ -n "${CC_SSH_LOCAL_FIXED_PATH_MARKER-}" ]; then
        case "$run_command" in
            *CC_SSH_ROOT_OBSERVE*) ;;
            *)
                if [ ! -e "$CC_SSH_LOCAL_FIXED_PATH_MARKER" ]; then
                    : >"$CC_SSH_LOCAL_FIXED_PATH_MARKER"
                    run_command_path=$CC_SSH_LOCAL_FIXED_PATH_ONCE
                fi
                ;;
        esac
    fi
    if [ "$run_is_root" -eq 0 ] && [ -n "${CC_SSH_BRIDGE_TEST_MODE-}" ]; then
    ...
    ' cc-ssh-dispatcher "$run_cwd" "$run_shell" "$run_command" "$run_dir/pid" "$run_command_path" \
    ...
    if [ "$run_is_root" -eq 1 ] &&
       ...
       [ ! -e "$FAKE_SSH_ROOT_RETARGET_MARKER" ]; then
        retarget_tmp=$FAKE_SSH_ROOT_RETARGET_LINK.cc-retarget.$$
...
handle_open() {
    ...
    case "$(cat "$command_file")" in
        *CC_SSH_ROOT_OBSERVE*) record_test_observation "$command_file" ;;
...
send_text HELLO_ACK 0 'protocol=cc-ssh-dispatcher/1;shell=sh;'
while read_frame; do
    case "$frame_kind" in
        HELLO) send_text HELLO_ACK "$frame_id" 'protocol=cc-ssh-dispatcher/1;shell=sh;' ;;
            ...
            if [ -f "$cancel_dir/pid" ]; then
                ...
                case "$cancel_pid" in
                    ...
                    *)
                        ...
                        setsid sh -c 'sleep 0.05; kill -KILL -"$1" 2>/dev/null || true' cc-ssh-killer "$cancel_pid" &
                        ;;
                esac
            fi
            rm -f "$frame_path"

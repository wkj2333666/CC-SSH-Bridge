...
impl Config {
    ...
    pub fn load_default() -> BridgeResult<LoadedConfig> {
        let (path, from_environment, warning) = match std::env::var_os("CC_SSH_BRIDGE_CONFIG") {
            Some(path) => (
                PathBuf::from(path),
                true,
                Some(
                    "CC_SSH_BRIDGE_CONFIG is trusted execution-authority input; verify its source and permissions"
                        .to_owned(),
                ),
            ),
    ...
    /// Load bridge configuration and merge concrete aliases discovered from
    /// the user's OpenSSH configuration without replacing explicit profiles.
...
fn default_config_path() -> BridgeResult<PathBuf> {
    ...
    Ok(base.join("cc-ssh-bridge").join("config.toml"))
}

...
use cc_ssh_bridge::job_protocol::{
    JOB_RECORD_VERSION, JobControlRequest, JobControlResponse, JobId, JobLogsRequest,
    JobRequestRecord, JobShell, JobState,
};
use cc_ssh_bridge::remote_helper_protocol::{Frame, FrameKind, read_frame, write_frame};
use cc_ssh_bridge::remote_job_runner::{JobStore, run_job_at};
...
fn helper_path() -> PathBuf {
    std::env::var("CARGO_BIN_EXE_cc-ssh-bridge-helper")
        .or_else(|_| std::env::var("CARGO_BIN_EXE_cc_ssh_bridge_helper"))
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("target/debug/cc-ssh-bridge-helper")
        })
}
...
fn helper_preserves_streams_and_exit_status() {
    ...
    assert!(
        String::from_utf8(hello.payload)
            .unwrap()
            .contains("protocol=cc-ssh-helper/1;version=1;")
    );
...
fn task15_job_store_modes_state_and_logs_are_durable() {
    ...
    let job_dir = home
        .path()
        .join(".local/state/cc-ssh-bridge/jobs")
...
fn task15_job_store_remains_pinned_after_the_visible_jobs_path_is_replaced() {
    ...
    let store = JobStore::open_at(home.path()).unwrap();
    let visible_jobs = home.path().join(".local/state/cc-ssh-bridge/jobs");
...
fn task15_job_survives_initiating_helper_and_is_visible_to_a_fresh_helper() {
    ...
    assert!(matches!(
        ...
        JobControlResponse::Started(ref state)
            if state.job_id == request.job_id
                && matches!(state.state, JobState::Running | JobState::Succeeded)
    ));
    send_frame(
        &mut first_input,
        Frame {

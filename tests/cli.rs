...
fn task9_hosts_add_show_list_remove_round_trip_and_save_mode_0600() {
    ...
    let loaded = cc_ssh_bridge::config::Config::load(&config).unwrap();
    ...
    bridge_command(&config, private.path())
        ...
        .success();
    assert!(
        cc_ssh_bridge::config::Config::load(&config)
            .unwrap()
            .hosts
            .is_empty()
    );
...
fn task9_fresh_config_creates_each_missing_parent_privately_without_global_config_flag() {
    ...
    Command::new(env!("CARGO_BIN_EXE_cc-ssh-bridge"))
        ...
        .assert()
        .code(2)
        .stderr("usage: cc-ssh-bridge mcp\n");
}
...
async fn task9_local_executor_timeout_kills_the_process_group_promptly() {
    ...
    .await
    .unwrap_err();
    assert_eq!(error.code, cc_ssh_bridge::ErrorCode::CommandTimeout);
...
async fn task9_local_executor_output_limit_wins_and_kills_a_term_ignoring_group() {
    ...
    .await
    .unwrap_err();
    assert_eq!(error.code, cc_ssh_bridge::ErrorCode::OutputLimit);
    assert!(started.elapsed() < Duration::from_secs(1));
}
...
fn install_fixture(options: InstallFixtureOptions) -> InstallFixture {
    ...
    let binary = bundle.join("bin/cc-ssh-bridge");
    ...
    let log = private.path().join("cc.log");
    for directory in [
    ...
    fs::write(
        &mcp_manifest,
        br#"{"mcpServers":{"ssh-bridge":{"command":"./bin/cc-ssh-bridge","args":["mcp"]}}}"#,
    )
    .unwrap();
    fs::write(
    ...
    let remove_body = if options.remove_failure_after_delete {
        format!("rm -f {}; exit 9", quote(&cc_state))
    } else {
        format!("rm -f {}", quote(&cc_state))
    };
    let script = format!(
...
async fn task9_installer_is_dry_run_by_default_then_applies_idempotently() {
    ...
    assert!(!dry.applied);
    assert!(!fixture.cc_state.exists());
...
async fn task9_installer_refuses_unrelated_mcp_and_differentiates_get_failure() {
    ...
    let failed = install_fixture(InstallFixtureOptions {
        get_failure: Some("permission denied reading Claude Code config"),
...
#[tokio::test]
async fn task9_installer_bounds_a_hung_cc_cli_call() {
...
#[tokio::test]
async fn task9_installer_accepts_trusted_root_owned_cc_executable() {
    let mut fixture = install_fixture(InstallFixtureOptions::default());
    fixture.layout.cc_executable = fs::canonicalize("/bin/true").unwrap();
    let metadata = fs::metadata(&fixture.layout.cc_executable).unwrap();
    assert_eq!(
...
#[tokio::test]
async fn task9_installer_accepts_claude_path_symlink_to_a_trusted_executable() {
    let mut fixture = install_fixture(InstallFixtureOptions::default());
    let linked_cc = fixture._private.path().join("linked-claude");
    symlink(&fixture.layout.cc_executable, &linked_cc).unwrap();
    fixture.layout.cc_executable = linked_cc;
...
#[tokio::test]
async fn task9_installer_rejects_claude_path_symlink_to_an_unsealed_writable_target() {
    let mut fixture = install_fixture(InstallFixtureOptions::default());
    fs::set_permissions(fixture._private.path(), fs::Permissions::from_mode(0o755)).unwrap();
    ...
    let unsafe_cc = unsafe_directory.join("claude");
    fs::copy(&fixture.layout.cc_executable, &unsafe_cc).unwrap();
    fs::set_permissions(&unsafe_cc, fs::Permissions::from_mode(0o700)).unwrap();
    let linked_cc = fixture._private.path().join("linked-claude");
    symlink(&unsafe_cc, &linked_cc).unwrap();
    fixture.layout.cc_executable = linked_cc;
...
#[tokio::test]
async fn task9_installer_validates_the_rust_package_chain_before_cc_mutation() {
...
async fn task9_installer_rolls_back_only_its_mcp_when_skill_creation_races() {
    ...
    assert!(
        ...
        "race target missing; claude log: {}",
...
#[tokio::test]
async fn task9_installer_rolls_back_matching_mcp_left_by_failed_cc_add() {
    let fixture = install_fixture(InstallFixtureOptions {
        fail_after_add: true,
        ..InstallFixtureOptions::default()
    });
...
async fn task9_apply_uses_one_private_cross_bundle_lock_and_rechecks_after_locking() {
    ...
    let lock = fixture
        ...
        .nth(3)
        .unwrap()
        .join(".cc-ssh-bridge.install.lock");
...
#[tokio::test]
async fn task9_apply_refuses_a_symlink_transaction_lock_before_cc_mutation() {
    let fixture = install_fixture(InstallFixtureOptions::default());
    let lock = fixture
        .layout
        .skill_target
...
#[tokio::test]
async fn task9_failed_cc_remove_that_did_remove_is_compensated() {
    let fixture = install_fixture(InstallFixtureOptions {
...
#[tokio::test]
async fn task9_destination_ancestors_are_fully_preflighted_before_claude_add() {
    let fixture = install_fixture(InstallFixtureOptions::default());
...
#[tokio::test]
async fn task9_unwritable_existing_destination_is_rejected_before_cc_is_called() {
    let fixture = install_fixture(InstallFixtureOptions::default());
    ...
    assert!(
        !fs::read_to_string(&fixture.log)
            .unwrap_or_default()
            .contains("mcp"),
        "destination failure must precede every Claude Code CLI call"
    );
    assert!(!fixture.cc_state.exists());
}
...
async fn task9_partial_destination_directory_creation_is_rolled_back() {
    ...
    assert!(
        !fs::read_to_string(&fixture.log)
            .unwrap_or_default()
            .contains("mcp"),
        "predictable ENAMETOOLONG must be detected before Claude Code CLI"
    );
    assert!(
        !state.exists(),

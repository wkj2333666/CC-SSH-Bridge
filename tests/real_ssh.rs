...
use cc_ssh_bridge::{ErrorCode, quote};
        ...
        fs::write(
            ...
            format!(
                concat!(
                    ...
                    "    SetEnv CC_TEST_NO_BASH=0\n",
                    "Host {sh_host}\n",
                    "    UserKnownHostsFile {known_hosts}\n",
                    "    SetEnv CC_TEST_NO_BASH=1\n",
                    "Host {wrong_host}\n",
...
async fn real_localhost_sshd_covers_transport_shell_files_mutation_and_cancellation() {
    let required = std::env::var("CC_SSH_BRIDGE_REQUIRE_REAL_SSH").as_deref() == Ok("1");

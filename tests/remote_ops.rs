...
use cc_ssh_bridge::job_protocol::{JobId, MAX_JOB_LABEL_BYTES, MAX_JOB_LOG_PAGE_BYTES};
use cc_ssh_bridge::output::OutputStore;
use cc_ssh_bridge::output::StreamKind;
use cc_ssh_bridge::remote::{
...
use cc_ssh_bridge::{BridgeError, ErrorCode};
...
fn fixture_with_probed_login_shell(
    ...
    write_executable(
        ...
        format!(
            ...
            cc_ssh_bridge::quote::shell_word(&format!(
...
async fn task8_retention_spool_accepts_exact_serialized_limit_and_rejects_first_byte_over() {
    ...
    let (_runtime, _runner, bridge) = fixture(root.path(), true);
    let remote_context = cached_context(&bridge).await;
    let exact = "x".repeat(cc_ssh_bridge::MAX_OUTPUT_BYTES as usize - 2);
    let reference = bridge
        .retain_serialized_detail(
            RetentionProvenance::Remote(remote_context.clone()),
            exact,
    ...
    let page = bridge
        .output_read(
            cc_ssh_bridge::remote::OutputReadRequest {
                output_ref: reference.as_str().to_owned(),
                stream: StreamKind::Stdout,
                offset: cc_ssh_bridge::MAX_OUTPUT_BYTES - 1,
                max_bytes: 1,
            },
            CancellationToken::new(),
        )
    ...
    let over = "x".repeat(cc_ssh_bridge::MAX_OUTPUT_BYTES as usize - 1);
    let error = bridge
        .retain_serialized_detail(
...
async fn task78_remote_run_requires_canonical_bounded_base64_before_command_child() {
    ...
    let remote = tempfile::TempDir::new().unwrap();
    let oversized = STANDARD.encode(vec![0; cc_ssh_bridge::MAX_WRITE_BYTES + 1]);
    ...
    for (command, expected_status) in [("exit 0", 0), ("exit 7", 7)] {
        ...
        let request = RemoteRunRequest {
            ...
            stdin: Some(RunStdin {
                ...
                value: "x".repeat(cc_ssh_bridge::MAX_WRITE_BYTES),
            }),
        };
...
fn task78_error_shell_metadata_is_closed() {
    ...
    error.details.shell = Some(cc_ssh_bridge::error::ErrorShellMetadata {
        kind: "sh".to_owned(),
        version: None,
        fallback: true,
    });
...
fn task78_domain_error_remote_context_helper_fills_only_missing_safe_fields() {
    use cc_ssh_bridge::error::{ErrorShellMetadata, attach_available_remote_context};
...
async fn task78_patch_second_snapshot_cancellation_keeps_first_snapshot_context() {
    ...
    write_executable(
        ...
        format!(
            ...
            cc_ssh_bridge::quote::shell_word(second_started.to_str().unwrap()).unwrap(),
        ),
    );
    let path = OsString::from(format!(
        "{}:/usr/local/bin:/usr/bin:/bin",
...
async fn assert_guard_pins_replaced_root(raw: bool) {
    ...
    write_executable(
        ...
        format!(
            ...
            cc_ssh_bridge::quote::shell_word(marker.to_str().unwrap()).unwrap(),
            cc_ssh_bridge::quote::shell_word(root.to_str().unwrap()).unwrap(),
            cc_ssh_bridge::quote::shell_word(pinned.to_str().unwrap()).unwrap(),
            cc_ssh_bridge::quote::shell_word(root.to_str().unwrap()).unwrap(),
            cc_ssh_bridge::quote::shell_word(marker.to_str().unwrap()).unwrap(),
        ),
    );
...
async fn raw_remote_run_preserves_the_callers_configured_locale() {
    ...
    let (_runtime, _runner, bridge) = fixture_with_options(
        ...
        false,
        None,
        &[("LC_ALL", OsString::from("cc_TEST.UTF-8"))],
    );
    let result = bridge
        .run(
            RemoteRunRequest {
        ...
        .unwrap();
    assert_eq!(result.exit_status, 0);
    assert_eq!(result.stdout.head.value, "cc_TEST.UTF-8");
}
...
async fn login_run_uses_the_probed_non_posix_account_shell_and_ignores_shell_environment() {
    ...
    write_executable(
        ...
        format!(
            "#!/bin/sh\n: >{}\nexit 92\n",
            cc_ssh_bridge::quote::shell_word(evil_marker.to_str().unwrap()).unwrap()
        ),
    );
...
async fn login_guard_does_not_enable_nounset_for_the_user_payload() {
    ...
    let result = bridge
        .run(
            RemoteRunRequest {
                host: "dev".to_owned(),
                command: "printf '<%s>' \"$CC_SSH_BRIDGE_UNSET\"".to_owned(),
                cwd: None,
                shell: RunShell::Login,
                timeout_ms: None,
                stdin: None,
...
fn task6_request_result_and_error_progress_shapes_are_closed() {
    ...
    let error = BridgeError {
        ...
        retryable: false,
        details: cc_ssh_bridge::ErrorDetails {
...
#[tokio::test]
async fn apply_patch_envelope_preparse_rejection_starts_no_ssh_process() {
    let remote = tempfile::TempDir::new().unwrap();
    let controls = tempfile::TempDir::new().unwrap();
...
#[tokio::test]
async fn apply_patch_envelope_add_update_and_delete_use_existing_mutation_pipeline() {
    let remote = tempfile::TempDir::new().unwrap();
...
#[tokio::test]
async fn apply_patch_envelope_update_of_complete_cached_base_stays_local() {
    let remote = tempfile::TempDir::new().unwrap();
...
async fn task6_snapshot_cancel_and_abort_remove_internal_spools() {
    ...
    let runtime_directory = runtime.path().join("cc-ssh-bridge");
    let cancel = CancellationToken::new();
...
async fn task6_snapshot_semantic_sentinel_requires_the_exact_nofollow_forms() {
    ...
    write_executable(
        &shim.path().join("stat"),
        b"#!/bin/sh\ncase \" $* \" in *\" -L --printf=%f:%u:%a:%s:%d:%i:%h\\n -- \"*cc-sentinel-patch-snapshot*/parent-link*) printf '41c0:0:700:0:1:2:1:extra\\n'; exit 0;; esac\nexec /usr/bin/stat \"$@\"\n",
    );
    let path = OsString::from(format!(
        "{}:/usr/local/bin:/usr/bin:/bin",
        shim.path().display()
...
async fn task6_snapshot_accepts_exact_write_limit_rejects_plus_one_and_cleans_spools() {
    let remote = tempfile::TempDir::new().unwrap();
    let exact = b"x\n".repeat(cc_ssh_bridge::MAX_WRITE_BYTES / 2);
...
async fn task6_aggregate_base_and_output_budgets_accept_exact_and_reject_plus_one() {
    let remote = tempfile::TempDir::new().unwrap();
    let half = b"x\n".repeat(cc_ssh_bridge::MAX_WRITE_BYTES / 4);
...
async fn task6_second_snapshot_reprobe_physical_root_drift_is_zero_mutation_conflict() {
    ...
    write_executable(
        &shim.path().join("stat"),
        format!(
            "#!/bin/sh\ncase \" $* \" in *\" -L --printf=%f:%u:%a:%s:%d:%i:%h\\n -- \"*cc-sentinel-patch-snapshot*/parent-link*) marker={}; count=$(/usr/bin/cat \"$marker\" 2>/dev/null || printf 0); count=$((count + 1)); printf %s \"$count\" >\"$marker\"; if [ \"$count\" -eq 2 ]; then /usr/bin/ln -sfn -- {} {}; printf '41c0:0:700:0:1:2:1:extra\\n'; exit 0; fi;; esac\nexec /usr/bin/stat \"$@\"\n",
            cc_ssh_bridge::quote::shell_word(count.to_str().unwrap()).unwrap(),
            cc_ssh_bridge::quote::shell_word(second_root.to_str().unwrap()).unwrap(),
            cc_ssh_bridge::quote::shell_word(active_root.to_str().unwrap()).unwrap(),
        ),
    );
    let path = OsString::from(format!(
        "{}:/usr/local/bin:/usr/bin:/bin",
...
async fn task6_assert_cached_root_drift_is_definite_conflict(delete: bool) {
    ...
    write_executable(
        ...
        format!(
            ...
            cc_ssh_bridge::quote::shell_word(expected.to_str().unwrap()).unwrap(),
            cc_ssh_bridge::quote::shell_word(hash_count.to_str().unwrap()).unwrap(),
            cc_ssh_bridge::quote::shell_word(snapshot_ready.to_str().unwrap()).unwrap(),
            cc_ssh_bridge::quote::shell_word(gate.to_str().unwrap()).unwrap(),
        ),
    );
    write_executable(
        &shim.path().join("stat"),
        format!(
            "#!/bin/sh\ncase \" $* \" in *\" --printf=%f -- \"*cc-sentinel-stat.*) marker={}; if [ ! -e \"$marker\" ]; then : >\"$marker\"; printf bad; exit 0; fi;; esac\nexec /usr/bin/stat \"$@\"\n",
            cc_ssh_bridge::quote::shell_word(stale_stat.to_str().unwrap()).unwrap(),
        ),
    );
    let path = OsString::from(format!(
        "{}:/usr/local/bin:/usr/bin:/bin",
...
async fn task6_postspawn_cancel_on_second_mutation_marks_only_current_unknown_and_cleans_up() {
    ...
    write_executable(
        &shim.path().join("dd"),
        format!(
            "#!/bin/sh\ncase \" $* \" in *\" of=./.cc-ssh-bridge.\"*\" bs=262144 \"*\" oflag=nofollow \"*) marker={}; count=$(/usr/bin/cat \"$marker\" 2>/dev/null || printf 0); count=$((count + 1)); printf %s \"$count\" >\"$marker\"; if [ \"$count\" -eq 2 ]; then : >{}; exec /usr/bin/cat {}; fi;; esac\nexec /usr/bin/dd \"$@\"\n",
            cc_ssh_bridge::quote::shell_word(count.to_str().unwrap()).unwrap(),
            cc_ssh_bridge::quote::shell_word(ready.to_str().unwrap()).unwrap(),
            cc_ssh_bridge::quote::shell_word(gate.to_str().unwrap()).unwrap(),
        ),
    );
    let path = OsString::from(format!(
...
async fn task5_pre_stdin_closed_records_survive_large_input_early_close() {
    ...
    std::fs::write(
        &stat,
        "#!/bin/sh\ncase \" $* \" in *cc-sentinel-safe-write*parent-link*) printf '41c0:0:700:0:1:2:1:extra\\n'; exit 0;; esac\nexec /usr/bin/stat \"$@\"\n",
    )
    .unwrap();
...
async fn task5_stale_write_sentinel_invalidates_only_a_future_request() {
    ...
    std::fs::write(
        &stat,
        format!(
            "#!/bin/sh\ncase \" $* \" in *cc-sentinel-safe-write*parent-link*) marker={}; if [ ! -e \"$marker\" ]; then : >\"$marker\"; printf '41c0:0:700:0:1:2:1:extra\\n'; exit 0; fi;; esac\nexec /usr/bin/stat \"$@\"\n",
            cc_ssh_bridge::quote::shell_word(marker.to_str().unwrap()).unwrap()
        ),
    )
    .unwrap();
...
async fn task5_write_exact_form_sentinel_matrix_is_semantic_and_future_only() {
    ...
    let cases = [
        Case {
            ...
            tool: "stat",
            rule: r#"case " $* " in
  *" -L --printf=%f:%u:%a:%s:%d:%i:%h\n -- "*cc-sentinel-safe-write*/work/parent-link*)
    if [ ! -e "$marker" ]; then : >"$marker"; printf '41ed:0:700:0:0:0:2\n'; exit 0; fi;;
        ...
        Case {
            ...
            tool: "stat",
            rule: r#"case " $* " in
  *" --printf=%f:%u:%a:%s:%d:%i:%h\n -- "*cc-sentinel-safe-write*/work/.cc-ssh-bridge.*)
    if [ ! -e "$marker" ]; then : >"$marker"; printf '8180:0:600:8:0:0:1\n'; exit 0; fi;;
        ...
        Case {
            ...
            tool: "stat",
            rule: r#"case " $* " in
  *" --printf=%f:%u:%a:%s:%d:%i:%h\n -- "*cc-sentinel-safe-write*/work/link*)
    if [ ! -e "$marker" ]; then : >"$marker"; printf '8180:0:600:7:0:0:1\n'; exit 0; fi;;
        ...
        Case {
            ...
            tool: "dd",
            rule: r#"case " $* " in
  *cc-sentinel-safe-write*/work/.cc-ssh-bridge.*bs=262144*status=none*conv=notrunc*oflag=nofollow*)
    if [ ! -e "$marker" ]; then : >"$marker"; exec /usr/bin/dd of=/dev/null bs=262144 status=none; fi;;
        ...
        Case {
            ...
            tool: "dd",
            rule: r#"case " $* " in
  *cc-sentinel-safe-write*/work/.cc-ssh-bridge.*bs=262144*status=none*iflag=nofollow*)
  ...
  exit 0
        ...
        Case {
            ...
            tool: "ln",
            rule: r#"case " $* " in
  *" -T -- "*cc-sentinel-safe-write*/work/.cc-ssh-bridge.*cc-sentinel-safe-write*/work/created*)
    ...
    if [ -e "$destination" ] && [ ! -e "$marker" ]; then : >"$marker"; exit 0; fi;;
        ...
        Case {
            ...
            tool: "mv",
            rule: r#"case " $* " in
  *" -T -- "*cc-sentinel-safe-write*/work/.cc-ssh-bridge.*cc-sentinel-safe-write*/work/replaced*)
    if [ ! -e "$marker" ]; then : >"$marker"; exit 0; fi;;
        ...
        Case {
            ...
            tool: "rm",
            rule: r#"case " $* " in
  *" -f -- "*cc-sentinel-safe-write*/work/created*)
    if [ ! -e "$marker" ]; then : >"$marker"; exit 0; fi;;
esac"#,
        },
    ];
    ...
    for case in cases {
        ...
        std::fs::write(
            ...
            format!(
                "#!/bin/sh\nmarker={}\narmed={}\n{}\nexec /usr/bin/{} \"$@\"\n",
                cc_ssh_bridge::quote::shell_word(marker.to_str().unwrap()).unwrap(),
                cc_ssh_bridge::quote::shell_word(armed.to_str().unwrap()).unwrap(),
                case.rule,
                case.tool,
            ),
        )
        ...
        if case.tool == "sha256sum" {
            ...
            std::fs::write(
                &stat,
                format!(
                    "#!/bin/sh\ncase \" $* \" in *\" --printf=%f:%u:%a:%s:%d:%i:%h\\n -- \"*cc-sentinel-safe-write*/work/.cc-ssh-bridge.*) : >{};; esac\nexec /usr/bin/stat \"$@\"\n",
                    cc_ssh_bridge::quote::shell_word(armed.to_str().unwrap()).unwrap(),
                ),
            )
            .unwrap();
...
async fn task5_staging_symlink_attack_never_modifies_referent() {
    ...
    std::fs::write(
        &dd,
        format!(
            "#!/bin/sh\ncase \" $* \" in *\" of=./.cc-ssh-bridge.\"*bs=262144*oflag=nofollow*) target=; for argument do case \"$argument\" in of=*) target=${{argument#of=}};; esac; done; /usr/bin/rm -f -- \"$target\"; /usr/bin/ln -s -- {} \"$target\"; exec /usr/bin/dd \"$@\";; esac\nexec /usr/bin/dd \"$@\"\n",
            cc_ssh_bridge::quote::shell_word(outside.to_str().unwrap()).unwrap()
        ),
    )
    .unwrap();
...
async fn task5_cleanup_signal_removes_remote_stage_and_local_spools() {
    ...
    std::fs::write(
        &dd,
        format!(
            "#!/bin/sh\ncase \" $* \" in *\" of=./.cc-ssh-bridge.\"*bs=262144*oflag=nofollow*) : >{}; exec sleep 10;; esac\nexec /usr/bin/dd \"$@\"\n",
            cc_ssh_bridge::quote::shell_word(ready.to_str().unwrap()).unwrap(),
        ),
    )
    .unwrap();
    ...
    for race in ["identity", "mode", "hash"] {
        ...
        let action = match race {
            "identity" => format!(
                ...
                cc_ssh_bridge::quote::shell_word(
        ...
        let body = if race == "hash" {
            ...
            format!(
                "#!/bin/sh\ncase \" $* \" in *\" of=./.cc-ssh-bridge.\"*bs=262144*oflag=nofollow*) /usr/bin/dd \"$@\"; status=$?; {}; exit \"$status\";; esac\nexec /usr/bin/dd \"$@\"\n",
                action
            )
        };
...
async fn task5_target_created_during_upload_is_a_closed_conflict() {
    ...
    std::fs::write(
        &dd,
        format!(
            "#!/bin/sh\ncase \" $* \" in *\" of=./.cc-ssh-bridge.\"*bs=262144*oflag=nofollow*) /usr/bin/dd \"$@\"; status=$?; printf RACE >{}; exit \"$status\";; esac\nexec /usr/bin/dd \"$@\"\n",
            cc_ssh_bridge::quote::shell_word(raced_target.to_str().unwrap()).unwrap()
        ),
    )
    .unwrap();
    std::fs::set_permissions(&dd, std::fs::Permissions::from_mode(0o755)).unwrap();
...
async fn readonly_real_mismatch_retries_exactly_once_from_the_list_script() {
    ...
    std::fs::write(
        &find,
        format!(
            "#!/bin/sh\ncase \" $* \" in *cc-probe-find*) exec /usr/bin/find \"$@\";; *cc-sentinel-list-production*) if [ ! -e {} ]; then : >{}; printf 'corrupt\\000'; exit 0; fi;; esac\nexec /usr/bin/find \"$@\"\n",
            cc_ssh_bridge::quote::shell_word(marker.to_str().unwrap()).unwrap(),
            cc_ssh_bridge::quote::shell_word(marker.to_str().unwrap()).unwrap(),
        ),
    )
...
async fn readonly_stale_sentinel_retries_each_production_form_exactly_once() {
    ...
    let cases = [
        Case {
            tool: "find",
            sentinel: "cc-sentinel-list-production",
        ...
        Case {
            tool: "xargs",
            sentinel: "cc-sentinel-list-xargs",
            failure: "exit 0",
            operation: "list",
            rg: true,
            expected_commands: 2,
        ...
        Case {
            tool: "find",
            sentinel: "cc-sentinel-search-find",
            failure: "printf 'corrupt\\000'; exit 0",
            operation: "rg-search",
            rg: true,
            expected_commands: 3,
    ...
    for case in cases {
        ...
        std::fs::write(
            &executable,
            format!(
                "#!/bin/sh\ncase \"${{CC_SSH_SENTINEL:-}}: $*\" in *{}*) printf 'S\\n' >>{}; if [ ! -e {} ]; then : >{}; {}; fi;; esac\nexec {} \"$@\"\n",
                case.sentinel,
                cc_ssh_bridge::quote::shell_word(sentinel_log.to_str().unwrap()).unwrap(),
                cc_ssh_bridge::quote::shell_word(marker.to_str().unwrap()).unwrap(),
                cc_ssh_bridge::quote::shell_word(marker.to_str().unwrap()).unwrap(),
                case.failure,
                cc_ssh_bridge::quote::shell_word(real_tool.to_str().unwrap()).unwrap(),
            ),
        )
        .unwrap();
        std::fs::set_permissions(&executable, std::fs::Permissions::from_mode(0o755)).unwrap();
...
async fn readonly_stale_sentinel_that_remains_bad_is_capability_missing() {
    ...
    std::fs::write(
        &find,
        b"#!/bin/sh\ncase \" $* \" in *cc-sentinel-list-production*) printf 'corrupt\\000'; exit 0;; esac\nexec /usr/bin/find \"$@\"\n",
    )
    .unwrap();
    std::fs::set_permissions(&find, std::fs::Permissions::from_mode(0o755)).unwrap();
    let path = OsString::from(format!(
...
async fn readonly_sentinel_setup_failure_is_not_retried() {
    ...
    std::fs::write(
        &mktemp,
        b"#!/bin/sh\ncase \" $* \" in *cc-sentinel-bound*) exit 1;; esac\nexec /usr/bin/mktemp \"$@\"\n",
    )
    .unwrap();
...
async fn task5_five_hosts_write_four_mib_with_bounded_rss_and_complete_cleanup() {
    use base64::{Engine as _, engine::general_purpose::STANDARD};
    use cc_ssh_bridge::config::HostProfile;
    ...
    std::fs::write(
        &dd,
        format!(
            "#!/bin/sh\ncase \" $* \" in *\" of=./.cc-ssh-bridge.\"*bs=262144*oflag=nofollow*) ready={}; release={}; : >\"$ready/${{PWD##*/}}\"; while [ ! -e \"$release\" ]; do sleep 0.005; done;; esac\nexec /usr/bin/dd \"$@\"\n",
            cc_ssh_bridge::quote::shell_word(ready_directory.to_str().unwrap()).unwrap(),
            cc_ssh_bridge::quote::shell_word(release.to_str().unwrap()).unwrap(),
        ),
    )
    .unwrap();
    std::fs::set_permissions(&dd, std::fs::Permissions::from_mode(0o755)).unwrap();
...
async fn search_real_engine_failure_is_fixed_and_redacted() {
    ...
    std::fs::write(
        &grep,
        b"#!/bin/sh\ncase \" $* \" in *cc-probe-grep*|*cc-sentinel-grep*|*/dev/null*) exec /usr/bin/grep \"$@\";; esac\nprintf 'VERY_SECRET_ENGINE_DIAGNOSTIC\\n' >&2\nexit 2\n",
    )
    .unwrap();
...
async fn search_finds_the_only_match_after_the_old_candidate_prefix_bound() {
    ...
    std::fs::write(&target, b"CC_SEARCH_TAIL_SENTINEL = True\n").unwrap();
    let (_runtime, _runner, bridge) = fixture(remote.path(), false);
    ...
    let result = bridge
        .search(
            SearchRequest {
                host: "dev".into(),
                query: "CC_SEARCH_TAIL_SENTINEL".into(),
                path: Some(remote.path().to_str().unwrap().into()),
                globs: vec!["*.py".into()],
                max_results: Some(10),
                binary: Some(false),
...
async fn search_unknown_rg_event_is_protocol_error() {
    ...
    std::fs::write(
        &rg,
        format!(
            "#!/bin/sh\ncase \" $* \" in *cc-probe-rg*|*cc-sentinel-rg*|*/dev/null*) exec {} \"$@\";; esac\nprintf '%s\\n' '{{\"type\":\"mystery\",\"data\":{{}}}}'\nexit 0\n",
            cc_ssh_bridge::quote::shell_word(real_rg.to_str().unwrap()).unwrap()
        ),
    )
    .unwrap();
    std::fs::set_permissions(&rg, std::fs::Permissions::from_mode(0o755)).unwrap();

...
use cc_ssh_bridge::config::{Config, HostLimitOverrides, HostProfile};
use cc_ssh_bridge::mcp::stdio::{exact_tools_list_response_bytes, required_mcp_frame_bytes};
use cc_ssh_bridge::mcp::tools::{RemoteMcpTools, tool_definitions};
use cc_ssh_bridge::mcp::{
...
use cc_ssh_bridge::remote::{RemoteBridge, RemoteRunRequest, RunShell};
use cc_ssh_bridge::ssh::{RuntimePaths, SshRunner};
...
fn absolute(root: &std::path::Path, relative: &str) -> String {
...
fn roomy_context() -> ToolCallContext {
    ToolCallContext {
        ...
        wire_budget: WireBudget {
            ...
            compact_fallback_bytes: cc_ssh_bridge::mcp::maximum_compact_fallback_result_bytes(),
        },
    }
}
...
fn binary_command(config: &std::path::Path, runtime: &std::path::Path) -> Command {
    let mut command = Command::new(env!("CARGO_BIN_EXE_cc-ssh-bridge"));
    command
        .env("CC_SSH_BRIDGE_CONFIG", config)
...
fn task8_binary_lifecycle_smoke_exposes_exact_surface_without_leaks() {
    ...
    std::fs::create_dir_all(private.path().join(".ssh")).unwrap();
    std::fs::write(private.path().join(".ssh/config"), "Host dev\n").unwrap();
...
struct ProtocolSession {
    ...
    server: tokio::task::JoinHandle<cc_ssh_bridge::BridgeResult<()>>,
    next_id: u64,
}

impl ProtocolSession {
    async fn start(tools: RemoteMcpTools) -> Self {
        Self::start_with_frame(tools, cc_ssh_bridge::MAX_FRAME_BYTES).await
    }
...
async fn task8_complete_surface_all_nine_tools_are_real_json_rpc_calls() {
    ...
    let stated = session
        .call(
            "remote_stat",
            json!({"host":"dev","paths":[absolute(remote.path(), "binary.bin"), absolute(remote.path(), "missing.txt")]}),
        )
        .await;
    ...
    let searched = session
        .call(
            "remote_search",
            json!({"host":"dev","query":"$(touch SHOULD_NOT_EXIST)","path":remote.path()}),
        )
        .await;
    ...
    let read = session
        .call(
            "remote_read",
            json!({"host":"dev","paths":[absolute(remote.path(), "utf8.txt"), absolute(remote.path(), "binary.bin")],"max_bytes":4096}),
        )
        .await;
    ...
    let run = session
        .call(
            "remote_run",
            json!({"host":"dev","command":"printf RUN_SURFACE; dd if=/dev/zero bs=1024 count=300 2>/dev/null","cwd":remote.path(),"shell":"sh"}),
        )
        .await;
    ...
    let written = session
        .call(
            "remote_write",
            json!({
                "host":"dev","path":absolute(remote.path(), "created.txt"),"content":"WRITE_SURFACE\n",
...
async fn task8_shell_surface_reports_bash_default_and_explicit_sh() {
    ...
    let default_bash = session
        .call(
            "remote_run",
            json!({"host":"dev","command":"printf safe","cwd":remote.path()}),
        )
        .await;
...
async fn task8_shell_surface_missing_bash_rejects_before_command_child() {
    ...
    let run = session
        .call(
            "remote_run",
            json!({"host":"dev","command":"printf must-not-run","cwd":remote.path(),"shell":"bash"}),
        )
        .await;
...
async fn task8_shell_surface_login_metadata_and_local_timeout_are_explicit() {
    ...
    let run = session
        .call(
            "remote_run",
            json!({"host":"dev","command":"printf safe","cwd":remote.path(),"shell":"login"}),
        )
        .await;
    assert_eq!(
...
fn task8_binary_unknown_and_missing_modes_have_fixed_usage_and_exit_two() {
    ...
    let expected = "usage: cc-ssh-bridge mcp\n";
...
fn task8_binary_fatal_error_is_only_fixed_prefix_and_stable_code() {
    ...
    assert_eq!(
        String::from_utf8_lossy(&output.stderr),
        "cc-ssh-bridge fatal: INVALID_CONFIG\n"
    );
    assert!(
        !output
            .stderr
...
async fn task8_binary_remote_tools_constructor_accepts_exact_minimum_only() {
    ...
    let fallback = cc_ssh_bridge::mcp::maximum_compact_fallback_result_bytes();
...
fn task8_schema_has_exact_required_fields_and_advisory_bounds() {
    ...
    for required in ["Claude Code apply_patch", "unified diff", "absolute"] {
        assert!(
...
async fn task8_dispatch_fake_ssh_maps_read_search_run_write_and_patch_presentations() {
    ...
    let stated = call_json(
        &tools,
        "remote_stat",
        json!({"host":"dev", "paths":[absolute(remote.path(), "utf8.txt"), absolute(remote.path(), "binary.bin")]}),
    )
    .await;
    ...
    let searched = call_json(
        &tools,
        "remote_search",
        json!({"host":"dev", "query":"UTF8_SENTINEL", "path":remote.path()}),
    )
    .await;
    ...
    let run = call_json(
        &tools,
        "remote_run",
        json!({"host":"dev", "command":"printf RUN_SENTINEL", "cwd":remote.path(), "shell":"sh"}),
    )
    .await;
...
fn task8_error_rendering_real_compact_fallback_is_nonzero_and_fits_minimum() {
    ...
    assert!(required <= cc_ssh_bridge::MAX_FRAME_BYTES);
}
...
fn fixed_command_shapes(log: &std::path::Path) -> Vec<(String, String)> {
    command_records(log)
        .into_iter()
        .map(|(argv, command)| (argv, fixed_script_prefix(&command, " cc-ssh-bridge-op ")))
        .collect()
}
...
async fn task8_hostile_content_and_command_output_remain_single_response_data() {
    ...
    call_json(
        ...
        "remote_write",
        json!({
            "host":"dev","path":absolute(remote.path(), "warm"),"content":"warm","encoding":"utf8",
            "mode":{"kind":"create"}
        }),
    )
    .await;
    ...
    for (index, value) in task8_hostile_values(true).into_iter().enumerate() {
        ...
        let absolute_path = absolute(remote.path(), &path);
        let result = call_json(
            &tools,
            "remote_write",
            json!({
                "host":"dev","path":absolute_path,"content":value,"encoding":"utf8",
                "mode":{"kind":"create"}
            }),
        )
        .await;
    ...
    for value in output_values {
        let result = session
            .call(
                "remote_run",
                json!({
                    "host":"dev","command":"cat","cwd":remote.path(),"shell":"sh",
...
fn five_host_tools_fixture(
    ...
    let hosts = roots
        ...
        .collect();
    let config = Config {
        limits: cc_ssh_bridge::config::Limits::default(),
        hosts,
        ..Config::default()
    };
    let log = runtime_base.path().join("ssh.log");
...
async fn task8_five_hosts_pipeline_in_parallel_with_exact_context_and_no_sixth_call() {
    ...
    for (host, root) in &root_paths {
        let warm = call_json(
            &tools,
            "remote_run",
            json!({"host":host,"command":":","cwd":root,"shell":"sh"}),
        )
        .await;
...
async fn task8_cancel_process_mcp_reaches_group_under_250ms_and_service_recovers() {
    ...
    session
        .send(json!({
            ...
            "params":{"name":"remote_run","arguments":{
                "host":"dev","command":"printf NEVER","cwd":remote.path(),"shell":"sh"
            }}
        }))
        .await;
    ...
    assert_hostile_marker_absent(remote.path());
    assert_eq!(
        regular_file_count(runtime.path().join("cc-ssh-bridge").as_path()),
        0,
...
async fn task78_release_fake_call_p95_is_below_ten_milliseconds() {
    ...
    let arguments = json!({"host":"dev","command":":","cwd":remote.path(),"shell":"sh"});
    for _ in 0..WARM_CALLS {
...
fn compact_context() -> ToolCallContext {
    let compact = cc_ssh_bridge::mcp::maximum_compact_fallback_result_bytes();
    ToolCallContext {
        cancel: CancellationToken::new(),
        wire_budget: WireBudget {
...
fn task8_output_rss_64_mib_and_retained_models_stay_below_sixteen_mib() {
    const CHILD_ENV: &str = "CC_SSH_BRIDGE_MCP_OUTPUT_RSS_CHILD";
    ...
    let (_output_runtime, _log, output_tools) = fake_remote_tools_with_options(
        ...
        &[
            ...
            (
                ...
                OsString::from(cc_ssh_bridge::MAX_OUTPUT_BYTES.to_string()),
            ),

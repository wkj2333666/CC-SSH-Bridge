...
use cc_ssh_bridge::config::{
    ...
    discover_ssh_aliases_from, migrate_v1_text,
};
use cc_ssh_bridge::error::{BridgeError, ErrorCode};
use cc_ssh_bridge::path::RemotePath;
use cc_ssh_bridge::quote::{fixed_command, shell_word};
use cc_ssh_bridge::{MAX_FRAME_BYTES, MAX_OUTPUT_BYTES, MAX_READ_BYTES, MAX_WRITE_BYTES};
use proptest::prelude::*;
use tempfile::{NamedTempFile, TempDir};

const LOAD_DEFAULT_CHILD_SENTINEL: &str = "CC_SSH_BRIDGE_TEST_LOAD_DEFAULT_CHILD";
#[cfg(unix)]
const FIFO_CHILD_SENTINEL: &str = "CC_SSH_BRIDGE_TEST_FIFO_CHILD_PATH";
...
#[test]
fn config_discovers_concrete_ssh_aliases_through_bounded_includes() {
...
"#,
    ...
    let bridge_config = write_config(valid_config());
...
fn config_load_default_in_isolated_child_process() {
    ...
    match case.as_str() {
        "override" => {
            let expected = std::env::var_os("CC_SSH_BRIDGE_CONFIG").unwrap();
            ...
            assert!(
                loaded
                    ...
                    .as_deref()
                    .unwrap()
                    .contains("CC_SSH_BRIDGE_CONFIG is trusted execution-authority input")
            );
            assert!(loaded.config.hosts.is_empty());
        }
        "xdg" => {
            let expected = Path::new(&std::env::var_os("XDG_CONFIG_HOME").unwrap())
                .join("cc-ssh-bridge/config.toml");
        ...
        "home" => {
            let expected = Path::new(&std::env::var_os("HOME").unwrap())
                .join(".config/cc-ssh-bridge/config.toml");
            assert_eq!(loaded.source.path, expected);
            assert!(!loaded.source.from_environment);
            assert_eq!(loaded.source.warning, None);
...
fn default_config_path_prefers_xdg_then_falls_back_to_home() {
    ...
    let xdg_config = xdg.join("cc-ssh-bridge/config.toml");
    let home_config = home.join(".config/cc-ssh-bridge/config.toml");
    ...
    xdg_child
        .env_remove("CC_SSH_BRIDGE_CONFIG")

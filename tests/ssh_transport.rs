...
use cc_ssh_bridge::capability::{
    CAPABILITY_PROBE_SCRIPT, Capability, CapabilityCache, ShellKind, ShellRequest,
    parse_probe_output, select_shell,
};
use cc_ssh_bridge::config::{Config, HostProfile, Limits};
use cc_ssh_bridge::error::{BridgeError, ErrorCode};
use cc_ssh_bridge::output::{CaptureLimits, OutputReference, OutputStore, StreamKind};
use cc_ssh_bridge::path::RemotePath;
use cc_ssh_bridge::remote::{RemoteBridge, StatRequest};
use cc_ssh_bridge::ssh::{RunRequest, RuntimePaths, SshPolicy, SshRunner, build_ssh_argv};
use cc_ssh_bridge::{MAX_OUTPUT_BYTES, MAX_READ_BYTES, MAX_WRITE_BYTES};
...
const LONG_XDG_CHILD_SENTINEL: &str = "CC_SSH_BRIDGE_LONG_XDG_CHILD_EXPECTED";
const RESTRICTIVE_UMASK_CHILD_SENTINEL: &str = "CC_SSH_BRIDGE_RESTRICTIVE_UMASK_BASE";
...
fn policy(
    config: &cc_ssh_bridge::config::Config,
    paths: &RuntimePaths,
    identity: &str,
) -> SshPolicy {
...
fn bash_probe(requested: &str, physical: &str) -> Vec<u8> {
    format!(
        "CC_SSH_PROBE=1\0REQUESTED_ROOT={requested}\0ROOT={physical}\0ROOT_DEVICE=1\0ROOT_INODE=2\0SHELL_KIND=bash\0BASH_VERSION=5.2.15\0LOGIN_SHELL=/bin/sh\0TOOL_rg=1\0TOOL_dd_nofollow=1\0TOOL_timeout=0\0"
    )
    .into_bytes()
}
...
fn bash_probe_with_version(requested: &str, physical: &str, version: &str) -> Vec<u8> {
    format!(
        "CC_SSH_PROBE=1\0REQUESTED_ROOT={requested}\0ROOT={physical}\0ROOT_DEVICE=1\0ROOT_INODE=2\0SHELL_KIND=bash\0BASH_VERSION={version}\0LOGIN_SHELL=/bin/sh\0TOOL_rg=1\0TOOL_dd_nofollow=1\0TOOL_timeout=0\0"
    )
    .into_bytes()
}
...
fn sh_probe_with_login(requested: &str, physical: &str, login_shell: &str) -> Vec<u8> {
    format!(
        "CC_SSH_PROBE=1\0REQUESTED_ROOT={requested}\0ROOT={physical}\0ROOT_DEVICE=1\0ROOT_INODE=2\0SHELL_KIND=sh\0BASH_VERSION=\0LOGIN_SHELL={login_shell}\0TOOL_rg=0\0"
    )
    .into_bytes()
}
...
fn base_for_control_path_bytes(container: &TempDir, target: usize) -> std::path::PathBuf {
    ...
    let suffix = 1 + "cc-ssh-bridge".len() + 1 + CONTROL_FILENAME_BYTES;
...
fn discover_falls_back_to_short_tmp_runtime_when_xdg_exceeds_control_path_budget() {
    ...
    let expected = format!("/tmp/cc-ssh-bridge-{uid}");
    ...
    assert!(
        !long_base.join("cc-ssh-bridge").exists(),
...
fn explicit_runtime_base_rejects_unrepresentable_control_paths_before_creating_a_leaf() {
    ...
    for name in names {
        ...
        assert!(
            !base.join("cc-ssh-bridge").exists(),
...
fn runtime_paths_refuse_insecure_modes_and_symlinks() {
    ...
    let insecure = insecure_base.path().join("cc-ssh-bridge");
    ...
    symlink(&target, symlink_base.path().join("cc-ssh-bridge")).unwrap();
    ...
    let special_mode = special_mode_base.path().join("cc-ssh-bridge");
...
fn parser_fails_closed_for_malformed_duplicate_unknown_and_mismatched_records() {
    ...
    let cases: Vec<Vec<u8>> = vec![
        b"CC_SSH_PROBE=1\0REQUESTED_ROOT=/srv/project\0ROOT=/srv/project\0SHELL_KIND=sh\0BASH_VERSION=".to_vec(),
        b"CC_SSH_PROBE=1\0BROKEN\0REQUESTED_ROOT=/srv/project\0ROOT=/srv/project\0SHELL_KIND=sh\0BASH_VERSION=\0".to_vec(),
        b"CC_SSH_PROBE=1\0REQUESTED_ROOT=/srv/project\0ROOT=/srv/project\0ROOT=/srv/other\0SHELL_KIND=sh\0BASH_VERSION=\0".to_vec(),
        b"CC_SSH_PROBE=2\0REQUESTED_ROOT=/srv/project\0ROOT=/srv/project\0SHELL_KIND=sh\0BASH_VERSION=\0".to_vec(),
        b"CC_SSH_PROBE=1\0REQUESTED_ROOT=/srv/other\0ROOT=/srv/project\0SHELL_KIND=sh\0BASH_VERSION=\0".to_vec(),
        b"CC_SSH_PROBE=1\0REQUESTED_ROOT=/srv/project\0ROOT=relative\0SHELL_KIND=sh\0BASH_VERSION=\0".to_vec(),
        b"CC_SSH_PROBE=1\0REQUESTED_ROOT=/srv/project\0ROOT=/srv/./project\0SHELL_KIND=sh\0BASH_VERSION=\0".to_vec(),
        b"CC_SSH_PROBE=1\0REQUESTED_ROOT=/srv/project\0ROOT=/srv/project\0SHELL_KIND=sh\0BASH_VERSION=\0UNKNOWN=value\0".to_vec(),
        b"CC_SSH_PROBE=1\0REQUESTED_ROOT=/srv/project\0ROOT=/srv/project\0SHELL_KIND=sh\0BASH_VERSION=\0TOOL_curl=1\0".to_vec(),
        b"CC_SSH_PROBE=1\0REQUESTED_ROOT=/srv/project\0ROOT=/srv/project\0SHELL_KIND=sh\0BASH_VERSION=\0TOOL_rg=2\0".to_vec(),
        b"CC_SSH_PROBE=1\0REQUESTED_ROOT=/srv/project\0ROOT=/srv/project\0SHELL_KIND=bash\0BASH_VERSION=\0".to_vec(),
        b"CC_SSH_PROBE=1\0REQUESTED_ROOT=/srv/project\0ROOT=/srv/project\0SHELL_KIND=sh\0BASH_VERSION=5.2\0".to_vec(),
        b"CC_SSH_PROBE=1\0REQUESTED_ROOT=/srv/project\0ROOT=/srv/project\0SHELL_KIND=unknown\0BASH_VERSION=\0".to_vec(),
        b"CC_SSH_PROBE=1\0REQUESTED_ROOT=/srv/project\0ROOT=/srv/project\0SHELL_KIND=sh\0BASH_VERSION=\0\xff\0".to_vec(),
        b"CC_SSH_PROBE=1\0REQUESTED_ROOT=/srv/project\0ROOT=/srv/project\0SHELL_KIND=sh\0".to_vec(),
    ];
...
fn task78_physical_root_byte_bound_bash_version_and_spoofed_values_are_enforced() {
    ...
    let spoofed = "$(touch /tmp/cc-ssh-bridge-must-not-run)";
    let capability = parse_probe_output(
    ...
    let leader = {
        ...
        tokio::spawn(async move {
            cache
                .get_or_probe("dev", || async move {
                    old_started.notify_one();
                    std::future::pending::<cc_ssh_bridge::BridgeResult<Capability>>().await
                })
                .await
        })
    };
...
fn capability_probe_accepts_only_one_strict_nss_login_shell_record() {
    ...
    let valid_script = format!(
        "#!/bin/sh\nprintf %s {}\n",
        cc_ssh_bridge::quote::shell_word(&valid_record).unwrap()
    );
    assert_eq!(
    ...
    let empty_shell_script = format!(
        "#!/bin/sh\nprintf %s {}\n",
        cc_ssh_bridge::quote::shell_word(&empty_shell_record).unwrap()
    );
    assert_eq!(
    ...
    for rejected in [duplicate, trailing_empty, malformed, relative, oversized] {
        let script = format!(
            "#!/bin/sh\nprintf %s {}\n",
            cc_ssh_bridge::quote::shell_word(&rejected).unwrap()
        );
...
fn capability_probe_rejects_each_incompatible_exact_behavior() {
    ...
    let cases = [
        (
            "read_slice",
            "tail",
            "case \" $* \" in *cc-probe-read*) exit 64;; esac\nexec /usr/bin/tail \"$@\"\n",
        ),
        (
            "find_nul",
            "find",
            "case \" $* \" in *cc-probe-find*) exit 64;; esac\nexec /usr/bin/find \"$@\"\n",
        ),
        (
            "find_nul",
            "find",
            "case \" $* \" in *cc-probe-find-link*) /usr/bin/find \"$@\" | /usr/bin/sed 's/visible/changed/g'; exit 0;; esac\nexec /usr/bin/find \"$@\"\n",
        ),
        (
            "stat_printf",
            "stat",
            "case \" $* \" in *cc-probe-stat*) exit 64;; esac\nexec /usr/bin/stat \"$@\"\n",
        ),
        ("rg_json", "rg", "exit 64\n"),
        (
            "rg_json",
            "rg",
            "case \" $* \" in *cc-probe-rg-error*) exit 1;; *needle*cc-probe-rg*) shim_out=${TMPDIR:-/tmp}/cc-rg-shim.$$; /usr/bin/rg \"$@\" >\"$shim_out\"; shim_status=$?; /usr/bin/sed 's/\"line_number\":1/\"line_number\":9/g' \"$shim_out\"; rm -f \"$shim_out\"; exit \"$shim_status\";; esac\nexec /usr/bin/rg \"$@\"\n",
        ),
        (
            "grep_nul",
            "grep",
        ...
        (
            "xargs_nul",
            "xargs",
            "case \" $* \" in *cc-ssh-probe-xargs*) exit 64;; esac\nexec /usr/bin/xargs \"$@\"\n",
        ),
        (
            "search_bound",
            "head",
        ...
        (
            "search_bound",
            "mktemp",
            "case \" $* \" in *cc-probe-bound*) exit 64;; esac\nexec /usr/bin/mktemp \"$@\"\n",
        ),
        (
            "search_bound",
            "mkfifo",
        ...
        (
            "search_bound",
            "xargs",
            "case \" $* \" in *cc-ssh-probe-bound-xargs*) exit 0;; esac\nexec /usr/bin/xargs \"$@\"\n",
        ),
        (
            "search_bound",
            "rm",
            "case \" $* \" in *cc-probe-bound.*) exit 0;; esac\nexec /usr/bin/rm \"$@\"\n",
        ),
        (
            "safe_write",
            "stat",
            "case \" $* \" in *\" -L \"*cc-probe-safe-write/followed-parent-link*) exit 64;; esac\nexec /usr/bin/stat \"$@\"\n",
        ),
        (
            "safe_write",
            "mktemp",
            "case \" $* \" in *--tmpdir=*cc-probe-safe-write*.cc-ssh-bridge.*) exit 64;; esac\nexec /usr/bin/mktemp \"$@\"\n",
        ),
        (
            "safe_write",
            "dd",
            "case \" $* \" in *cc-probe-safe-write*bs=262144*oflag=nofollow*) exit 64;; esac\nexec /usr/bin/dd \"$@\"\n",
        ),
        (
            "safe_write",
            "dd",
            "case \" $* \" in *cc-probe-safe-write*bs=262144*iflag=nofollow*) /usr/bin/dd \"$@\"; exit 7;; esac\nexec /usr/bin/dd \"$@\"\n",
        ),
        (
            "safe_write",
            "stat",
            "case \" $* \" in *cc-probe-safe-write/.cc-ssh-bridge.*) printf '81a4:0:600:7:1:2:1:extra\\n'; exit 0;; esac\nexec /usr/bin/stat \"$@\"\n",
        ),
        (
            "safe_write",
            "stat",
            "case \" $* \" in *cc-probe-safe-write/.cc-ssh-bridge.*) printf '81a4:0:600:7:1:*:1\\n'; exit 0;; esac\nexec /usr/bin/stat \"$@\"\n",
        ),
        (
            "safe_write",
            "stat",
            "case \" $* \" in *\" --printf=%f:%u:%a:%s:%d:%i:%h\\n -- \"*cc-probe-safe-write*) line=$(/usr/bin/stat \"$@\") || exit $?; old_ifs=$IFS; IFS=:; set -- $line; IFS=$old_ifs; printf '%s:%s:%s:%s:123456789012345678901:123456789012345678901:%s\\n' \"$1\" \"$2\" \"$3\" \"$4\" \"$7\"; exit 0;; esac\nexec /usr/bin/stat \"$@\"\n",
        ),
        (
            "safe_write",
            "stat",
            "case \" $* \" in *\" --printf=%f:%u:%a:%s:%d:%i:%h\\n -- \"*cc-probe-safe-write*) line=$(/usr/bin/stat \"$@\") || exit $?; old_ifs=$IFS; IFS=:; set -- $line; IFS=$old_ifs; printf '%s:%s:%s:%s:18446744073709551616:18446744073709551616:%s\\n' \"$1\" \"$2\" \"$3\" \"$4\" \"$7\"; exit 0;; esac\nexec /usr/bin/stat \"$@\"\n",
        ),
        (
            "safe_write",
            "stat",
            "case \" $* \" in *\" --printf=%f:%u:%a:%s:%d:%i:%h\\n -- \"*cc-probe-safe-write/dd-link*) printf '8180:0:600:7:1:2:1\\n'; exit 0;; esac\nexec /usr/bin/stat \"$@\"\n",
        ),
        (
            "safe_write",
            "id",
        ...
        (
            "guarded_delete",
            "stat",
            "case \" $* \" in *\" -L \"*cc-probe-guarded-delete*) exit 64;; esac\nexec /usr/bin/stat \"$@\"\n",
        ),
        (
            "guarded_delete",
            "stat",
            "case \" $* \" in *cc-probe-guarded-delete*target:*) marker=${TMPDIR:-/tmp}/task5-delete-stat-count; count=$(/usr/bin/cat \"$marker\" 2>/dev/null || printf 0); count=$((count + 1)); if [ \"$count\" -eq 2 ]; then /usr/bin/rm -f \"$marker\"; printf 'a1ff:0:777:7:1:2:1\\n'; exit 0; fi; printf %s \"$count\" >\"$marker\";; esac\nexec /usr/bin/stat \"$@\"\n",
        ),
        (
            "guarded_delete",
            "dd",
            "case \" $* \" in *cc-probe-guarded-delete*bs=262144*iflag=nofollow*) marker=${TMPDIR:-/tmp}/task5-guarded-hash-count; count=$(/usr/bin/cat \"$marker\" 2>/dev/null || printf 0); count=$((count + 1)); if [ \"$count\" -eq 2 ]; then /usr/bin/rm -f \"$marker\"; exit 64; fi; printf %s \"$count\" >\"$marker\";; esac\nexec /usr/bin/dd \"$@\"\n",
        ),
        (
            "safe_write",
            "ln",
            "case \" $* \" in *cc-probe-safe-write*created*) exit 0;; esac\nexec /usr/bin/ln \"$@\"\n",
        ),
        (
            "safe_write",
            "ln",
            "case \" $* \" in *\" -T -- \"*cc-probe-safe-write*directory-link*) shift 2; exec /usr/bin/ln -- \"$@\";; esac\nexec /usr/bin/ln \"$@\"\n",
        ),
        (
            "safe_write",
            "mv",
            "case \" $* \" in *cc-probe-safe-write*replaced*) exit 0;; esac\nexec /usr/bin/mv \"$@\"\n",
        ),
        (
            "safe_write",
            "rm",
            "case \" $* \" in *\" -f -- \"*cc-probe-safe-write*created*) exit 0;; esac\nexec /usr/bin/rm \"$@\"\n",
        ),
        (
            "guarded_delete",
            "rm",
            "case \" $* \" in *cc-probe-guarded-delete*target:*) exit 0;; esac\nexec /usr/bin/rm \"$@\"\n",
        ),
    ];
...
fn task5_mutation_hash_probe_is_closed_and_restores_shell_state() {
    ...
    fs::write(
        &executable,
        format!(
            "#!/bin/sh\nmarker={}\ncount=$(/usr/bin/cat \"$marker\" 2>/dev/null || printf 0)\ncount=$((count + 1))\nprintf %s \"$count\" >\"$marker\"\nif [ \"$count\" -eq 2 ]; then printf 'CC_DD_STATUS=0\\nMALFORMED\\n'; fi\nexec /usr/bin/sha256sum \"$@\"\n",
            cc_ssh_bridge::quote::shell_word(count_path.to_str().unwrap()).unwrap()
        ),
    )
    .unwrap();
    fs::set_permissions(&executable, fs::Permissions::from_mode(0o755)).unwrap();
...
fn local_fixed_executes_the_real_capability_probe() {
    ...
    let scratch = TempDir::new().unwrap();
    let command = format!(
        "exec sh -c {} cc-ssh-probe {}",
        cc_ssh_bridge::quote::shell_word(CAPABILITY_PROBE_SCRIPT).unwrap(),
        cc_ssh_bridge::quote::shell_word(root.path().to_str().unwrap()).unwrap()
    );
...
async fn command_stdin_is_streamed_and_oversized_input_is_rejected_before_ssh() {
    ...
    oversized.stdin = Some(vec![0; cc_ssh_bridge::MAX_WRITE_BYTES + 1]);
    let error = fixture
        .runner
...
#[tokio::test]
async fn normal_completion_does_not_wait_for_pipe_inheriting_descendants() {
    let files = TempDir::new().unwrap();
    let pid_file = files.path().join("child.pid");
    let parent_exit = files.path().join("parent.exit");
    let fixture = task3_runner(
    ...
    .await
    .expect("normal completion waited for an orphaned stream")
    .unwrap();
    ...
    .await
    .expect("a completed orphaned request blocked the next request")
    .unwrap();
...
async fn task8_internal_capture_quota_is_shared_with_a_committed_command_spool() {
    ...
    let store = Arc::new(
        OutputStore::with_limits(
            &runtime,
            cc_ssh_bridge::config::MIN_GLOBAL_SPOOL_QUOTA_BYTES,
            1,
        )
        .unwrap(),
    );

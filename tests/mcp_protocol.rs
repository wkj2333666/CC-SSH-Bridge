...
use cc_ssh_bridge::error::ErrorShellMetadata;
use cc_ssh_bridge::mcp::stdio::{
...
use cc_ssh_bridge::mcp::{
    ...
    SUPPORTED_PROTOCOL_VERSIONS, ShutdownFuture, StrictJsonError, ToolAnnotations, ToolCallContext,
    ToolDefinition, ToolFuture, ToolService, WireBudget, duplicate_request_id_response,
    internal_error_response, invalid_params_response, invalid_request_id_response,
    invalid_request_response, maximum_compact_fallback_result_bytes, method_not_found_response,
    parse_error_response, parse_strict_json, request_too_large_response, result_response,
...
fn task7_min_frame_authoritative_future_renderer_projection_fits_compiled_floor() {
    ...
    let root = format!(
        "/{}",
        "\u{1}".repeat(cc_ssh_bridge::config::MAX_REMOTE_CONTEXT_ROOT_BYTES - 1)
    );
    assert_eq!(
        root.len(),
        cc_ssh_bridge::config::MAX_REMOTE_CONTEXT_ROOT_BYTES
    );
    let version = "\u{1}".repeat(cc_ssh_bridge::capability::MAX_SHELL_VERSION_BYTES);
    assert_eq!(
        version.len(),
        cc_ssh_bridge::capability::MAX_SHELL_VERSION_BYTES
    );
...
fn task7_min_frame_wire_budget_reserves_envelope_id_and_fallback_only() {
    ...
    assert_eq!(
        ...
        cc_ssh_bridge::mcp::MAX_REQUEST_ID_WIRE_BYTES
    );
...
async fn task7_adversarial_exact_eight_mib_and_plus_one_recovery() {
    let exact_request =
        serde_json::to_vec(&json!({"jsonrpc":"2.0","id":"exact","method":"ping","params":{}}))
            .unwrap();
    assert!(exact_request.len() < cc_ssh_bridge::MAX_FRAME_BYTES);

    let mut input = Vec::with_capacity(cc_ssh_bridge::MAX_FRAME_BYTES * 2 + 512);
    input.extend_from_slice(&exact_request);
    input.resize(cc_ssh_bridge::MAX_FRAME_BYTES, b' ');
    input.push(b'\n');
...
fn task7_wide_json_rss_array_fresh_child() {
    run_wide_json_rss_fresh_child(
        "CC_SSH_BRIDGE_WIDE_ARRAY_RSS_CHILD",
...
fn task7_wide_json_rss_object_fresh_child() {
    run_wide_json_rss_fresh_child(
        "CC_SSH_BRIDGE_WIDE_OBJECT_RSS_CHILD",

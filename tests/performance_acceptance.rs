...
fn roomy_context() -> ToolCallContext {
    ToolCallContext {
        ...
        wire_budget: WireBudget {
            result_bytes: cc_ssh_bridge::MAX_FRAME_BYTES,
...
async fn task13_release_edit_cache_latency_profile() {
    ...
    let fixture = edit_cache_fixture(
        cc_ssh_bridge::config::DEFAULT_EDIT_FLUSH_DELAY_MS,
        cc_ssh_bridge::config::DEFAULT_EDIT_FLUSH_THRESHOLD_BYTES,
    );
    ...
    assert_eq!(barrier["structuredContent"]["exit_status"], 0, "{barrier}");
    ...
    let timer = edit_cache_fixture(25, 1024 * 1024);
    ...
    let threshold = edit_cache_fixture(30_000, 16);
...
fn task13_release_edit_cache_rss_fresh_child() {
    const CHILD_ENV: &str = "CC_SSH_BRIDGE_TASK13_EDIT_CACHE_RSS_CHILD";
...
fn report_maximum_mcp_wire() {
    ...
    McpServer::new(service, cc_ssh_bridge::MAX_FRAME_BYTES).unwrap();
    eprintln!(
        ...
        cc_ssh_bridge::MAX_FRAME_BYTES,
        cc_ssh_bridge::MAX_FRAME_BYTES + 1
    );
    assert_eq!(cc_ssh_bridge::MAX_FRAME_BYTES, 8 * 1024 * 1024);
    assert!(tools_list_bytes <= cc_ssh_bridge::MAX_FRAME_BYTES);
    assert!(required_bytes <= cc_ssh_bridge::MAX_FRAME_BYTES);
}
...
fn task11_release_bounded_session_output_rss_fresh_child() {
    const CHILD_ENV: &str = "CC_SSH_BRIDGE_TASK11_OUTPUT_RSS_CHILD";
...
fn task11_release_zero_output_concurrent_rss_fresh_child() {
    const CHILD_ENV: &str = "CC_SSH_BRIDGE_TASK11_ZERO_OUTPUT_RSS_CHILD";
...
async fn zero_output_rss_child() {
    ...
    for _ in 0..CONCURRENCY {
        let result = call_json(
            &tools,
            "remote_run",
            json!({"host":"dev","cwd":"/srv/project","command":":","shell":"sh"}),
        )
        .await;
        assert_eq!(result["isError"], Value::Null, "{result}");
    }
...
fn task11_release_max_wide_array_rss_fresh_child() {
    run_wide_json_rss_fresh_child(
        "CC_SSH_BRIDGE_TASK11_WIDE_ARRAY_RSS_CHILD",
...
fn task11_release_max_wide_object_rss_fresh_child() {
    run_wide_json_rss_fresh_child(
        "CC_SSH_BRIDGE_TASK11_WIDE_OBJECT_RSS_CHILD",
...
fn task11_max_output_constant_matches_64_mib() {
    assert_eq!(cc_ssh_bridge::MAX_OUTPUT_BYTES, 64 * 1024 * 1024);
}

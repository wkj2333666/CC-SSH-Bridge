...
- the local Unix user running Claude Code and the bridge;
...
- local OpenSSH, `known_hosts`, private keys or agent, and explicit Claude Code approvals.
...
Remote operations are rooted at the server filesystem `/`; the bridge does not provide a per-host workspace sandbox. Remote reads can follow remote symlinks, just as commands run directly on the server can. The connection-time capability probe records the physical path and device/inode identity using a GNU- or BSD-compatible `stat` for diagnostics; it is not a per-request authorization guard and is not re-run on warm requests. Safe writes and patches additionally use no-follow path identity checks, snapshots, hashes, expected-content checks, and guarded commits, but the remote Unix account, permissions, container, forced command, or service policy remains the hard boundary.

No Claude Code credential, plugin, MCP server, or bridge daemon is placed on a server. On a supported Linux host the bridge stores only a versioned static helper beneath `~/.local/share/cc-ssh-bridge/helpers/`. Every directory and helper is private to the remote account, the helper must be a non-symlink mode-0700 file with the exact expected length and SHA-256, and installation uses a private lock plus a same-directory temporary file before replacement. Uploaded bytes are length-delimited stdin data and are never interpolated into shell source. All SSH authentication occurs in the local OpenSSH client.

Operational work uses one locally owned SSH child per configured alias. The selected helper process or shell dispatcher lives only for that SSH session; the cached helper file is inert between sessions. Each request is framed as data, starts a separate process group, and has independent stdout/stderr limits, timeout, and cancellation. Before any request is accepted, persistent-helper startup may fall back to a temporary helper, and temporary-helper startup may fall back to the shell dispatcher. After acceptance, transport failure is an unknown outcome and is never silently replayed through another mode. Results expose the selected `helper_mode`.
...
Connection setup also applies the configured `ConnectTimeout`. Ordinary SSH and SSHFS both inherit the two server-alive options exactly once; SSHFS additionally applies `reconnect` and never enables `allow_other`. Read-only access belongs in the remote account, filesystem, container, forced-command, or service policy rather than a bridge host profile.

At startup, the bridge reads concrete `Host` aliases from `~/.ssh/config`, follows `Include` files with bounded recursion and cycle detection, and ignores wildcard and negated entries. Discovered aliases use remote filesystem root `/` and the global operational limits. The first operation for an alias runs bounded `ssh -G` with the security-critical options and hashes the resulting configuration. That digest and the derived policy are cached for the bridge process; warm requests do not repeat `ssh -G`. A mismatch discovered during initial setup is non-retryable `INVALID_CONFIG`; restart the bridge only after reviewing an intentional local alias change. Host aliases are passed after `--`, and the MCP surface accepts no arbitrary SSH option.

Each MCP runner includes a private instance identity in the `ControlPath` hash. Independent Claude Code tasks therefore cannot attach to one another's OpenSSH master. The identity is not a credential; the containing runtime directory is already current-user-owned mode `0700`. Within one runner, the capability probe and persistent helper startup may reuse the same master during the five-second cold-start grace.
...
MCP filesystem paths and `remote_run.cwd` are required absolute remote paths; no task, SSH-home, previous-call, configured profile root, or implicit workspace is inherited. Paths, queries, globs, patch bodies, file content, and stdin are transported as data. Fixed remote programs use static scripts plus positional parameters. The direct human CLI follows the same absolute-path rule and converts each argv word with the bridge's bounded shell encoder.
...
- re-queries Claude Code after add/remove success, failure, or timeout and compensates observed partial mutation;
- removes only content matching the recorded installation identity.

The lock serializes this bridge's transactions. The local Unix user remains trusted: a separate process running as that same user can ignore the lock and directly edit Claude Code configuration. Final rechecks detect many such races, but this is not a hostile-same-UID isolation mechanism.
